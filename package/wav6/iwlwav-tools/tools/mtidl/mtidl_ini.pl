...
my $mtidl_val_byte_re = 'MTIDL_BYTEVAL'.$mtidl_val_arg_tuple_re;
my $mtidl_val_sbyte_re = 'MTIDL_SBYTEVAL'.$mtidl_val_arg_tuple_re;
my $mtidl_val_short_re = 'MTIDL_SHORTVAL'.$mtidl_val_arg_tuple_re;
my $mtidl_val_sshort_re = 'MTIDL_SSHORTVAL'.$mtidl_val_arg_tuple_re;
...
my $mtidl_array_byte_re = 'MTIDL_BYTEVAL_ARRAY'.$mtidl_array_arg_tuple_re;
my $mtidl_array_sbyte_re = 'MTIDL_SBYTEVAL_ARRAY'.$mtidl_array_arg_tuple_re;
my $mtidl_array_short_re = 'MTIDL_SHORTVAL_ARRAY'.$mtidl_array_arg_tuple_re;
my $mtidl_array_sshort_re = 'MTIDL_SSHORTVAL_ARRAY'.$mtidl_array_arg_tuple_re;

my $mtidl_array_long_re = 'MTIDL_LONGVAL_ARRAY'.$mtidl_array_arg_tuple_re;
my $mtidl_array_slong_re = 'MTIDL_SLONGVAL_ARRAY'.$mtidl_array_arg_tuple_re;
...
my $mtidl_array_2d_arg_tuple_re = '\(\s*('.$word_re.')\s*,\s*('.$int_re.')\s*,\s*('.$int_re.')\s*,\s*\"(.*?)\"\s*\)';

my $mtidl_array_2d_long_re = 'MTIDL_LONGVAL_ARRAY_2D'.$mtidl_array_2d_arg_tuple_re;
my $mtidl_array_2d_byte_re = 'MTIDL_BYTEVAL_ARRAY_2D'.$mtidl_array_2d_arg_tuple_re;
my $mtidl_array_2d_short_re = 'MTIDL_SHORTVAL_ARRAY_2D'.$mtidl_array_2d_arg_tuple_re;
...
sub fill_item_properties
{
  ...
  if ($do_offs)
  {
    # Use field "description" as value of the optional "next_idx_offs" fields property
    $idx_offs = shift;
    if ("$idx_offs" eq "\"\"") { $idx_offs = 0; }
    # if ($idx_offs) { ${$dictionary_ref}{"next_idx_offs"} = $idx_offs; }  # will be used by process_item_field
...
sub process_item_field
{
  ...
  # Apply $idx_offs to fields properties
  if ($idx_offs) { $field_properties{"next_idx_offs"} = $idx_offs; }

  if ($do_offs)
  {
    ${$dictionary_ref}{"offset"} = $offs_val;
    ...
    if ($idx_offs)
    {
        ...
        {
            $blk_offset = $offs_val;
            $blk_idx_max = $idx_offs / $element_size;
        }

        $blk_idx_cur += 1;
        $offs_val += $element_size;
        if ($blk_idx_cur == $blk_idx_max)
        {
            $blk_idx_cur = 0;
            $offs_val = $num_elements * $idx_offs + $blk_offset;
        }
    }
    else
    {
      $offs_val += $num_elements * $element_size;
    }
  }
...
sub process_array_2d_item_field
{
  my $dictionary_ref = shift;
  my $dimension1 = shift;
  my $dimension2 = shift;
  ...
  my $num_elements = $dimension1 * $dimension2;
  my $num_dimensions = 2;
  ...
  my %field_properties;
  $field_properties{"num_elements"} = $num_elements;
  $field_properties{"num_dimensions"} = $num_dimensions;
  $field_properties{"dimension2"} = $dimension2;
  $field_properties{"fract_size"} = $fract_size;
  $field_properties{"element_size"} = lookup_constant($element_size);
  $field_properties{"description"} = "\"".$description."\"";
  $field_properties{"binary_type"} = $binary_type;
...
sub process_item_line
{
  ...
  # Skip C preprocessor lines (with '#')
  return "" if($line =~ /^[#]/);

  dbg_print("\tprocess_item_line($line)\n");

  $line =~ s/$mtidl_val_byte_re/process_item_field($items_dictionary,   1, 1, "", $1, "byte",   "")/xemg;
  $line =~ s/$mtidl_val_sbyte_re/process_item_field($items_dictionary,  1, 1, "", $1, "sbyte",  "")/xemg;
  $line =~ s/$mtidl_val_short_re/process_item_field($items_dictionary,  1, 2, "", $1, "short",  "")/xemg;
  $line =~ s/$mtidl_val_sshort_re/process_item_field($items_dictionary, 1, 2, "", $1, "sshort", "")/xemg;
  ...
  $line =~ s/$mtidl_array_byte_re/process_item_field($items_dictionary,   $1, 1, "", $2, "byte",   "")/xemg;
  $line =~ s/$mtidl_array_sbyte_re/process_item_field($items_dictionary,  $1, 1, "", $2, "sbyte",  "")/xemg;
  $line =~ s/$mtidl_array_short_re/process_item_field($items_dictionary,  $1, 2, "", $2, "short",  "")/xemg;
  $line =~ s/$mtidl_array_sshort_re/process_item_field($items_dictionary, $1, 2, "", $2, "sshort", "")/xemg;

  $line =~ s/$mtidl_array_long_re/process_item_field($items_dictionary, $1, 4, "", $2, "long", "")/xemg;
  $line =~ s/$mtidl_array_slong_re/process_item_field($items_dictionary, $1, 4, "", $2, "slong", "")/xemg;
  ...
  $line =~ s/$mtidl_array_2d_long_re/process_array_2d_item_field($items_dictionary,  $2, $3, 4, "", $4, "long",  "")/xemg;
  $line =~ s/$mtidl_array_2d_byte_re/process_array_2d_item_field($items_dictionary,  $2, $3, 1, "", $4, "byte",  "")/xemg;
  $line =~ s/$mtidl_array_2d_short_re/process_array_2d_item_field($items_dictionary, $2, $3, 2, "", $4, "short", "")/xemg;
...
sub process_item
{
  ...
  $text =~ s/
             ...
             ($word_re)                                     #ID
             \s*,\s*
             (\"(.*?)\"|($word_re))\s*\)                    # Description (string) OR NextIdxOffset (value)
...
Usage:
    perl mtidl_ini.pl [--offset] <Output file name> <Input file name 1> ... <Input file name N>
        options:    --offset    - Calculate offset and add it to each "item" for every input file,
                                  all items of an input file describe a whole data block.
USAGE_END
...
my $mtidl_entry_re = 'MTIDL_ENTRY_START((?: .|\n)*?)MTIDL_ENTRY_END\s*\(\s*('.$word_re.')\s*\)';
...
print("\tParsing entry... ");
$text =~ s/$mtidl_entry_re/process_item(\@items_array, $1, $2)/xemg;
print("DONE.\n");
