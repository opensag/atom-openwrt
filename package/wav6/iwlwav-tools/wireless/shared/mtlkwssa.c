...
static void
_mtlk_node_name_from_sta_addr(IEEE_ADDR* sta_addr, char* name_buffer, size_t name_buffer_size)
{
...
static void
_mtlk_sta_flags_from_node_name(const char* sta_flags, BOOL* station_flags_array)
{
  char sta_auth_flag;

  if(sizeof(sta_auth_flag) > sscanf(sta_flags, MTLK_IRB_STA_FLAGS "%hhd", &sta_auth_flag))
  {
    *station_flags_array = FALSE;
  }
  else
  {
    *station_flags_array = (BOOL)sta_auth_flag;
  }
}

static void
_mtlk_wss_fill_station_flags_array(const char* sta_name, BOOL* station_flags_array)
{
  mtlk_node_t *sta_node = mtlk_node_alloc();

  *station_flags_array = FALSE;
  if(NULL != sta_node)
  {
    if(MTLK_ERR_OK == mtlk_node_attach(sta_node, sta_name))
    {
      if(0 < mtlk_node_get_nof_sons(sta_node))
      {
        mtlk_node_son_t *node_son = mtlk_node_son_get_first(sta_node);
        const char* son_name = mtlk_node_son_get_desc(sta_node, node_son);

        if(0 == strncmp(son_name, MTLK_IRB_STA_FLAGS, strlen(MTLK_IRB_STA_FLAGS)))
        {
          _mtlk_sta_flags_from_node_name(son_name, station_flags_array);
        }
      }
      mtlk_node_detach(sta_node);
    }
    mtlk_node_free(sta_node);
  }
}

static int
_mtlk_wss_fill_stations_array(mtlk_node_t* wlan_node, IEEE_ADDR *station_array, BOOL* station_flags_array, uint32 array_size)
{
  ...
  while(NULL != node_son)
  {
    ...
    if(0 == strncmp(son_name, MTLK_IRB_STA_NAME, strlen(MTLK_IRB_STA_NAME)))
    {
      if(num_stations < array_size)
      {
        ...
        _mtlk_wss_fill_station_flags_array(son_name, &station_flags_array[num_stations]);
        num_stations++;
      }
      else {
        WLOG_D("Buffer size (%d) is too small to return all connected peers", array_size);
        break;
      }
    }
                                   ...
                                   size_t name_buffer_length)
{
  ...
  if(NULL != wlan_node)
  {
    ...
    if(MTLK_ERR_OK == mtlk_node_attach(wlan_node, interface_name))
    {
      ...
      {
        wave_strcopy(name_buffer, parent_descr, name_buffer_length);
        res = MTLK_ERR_OK;
      }
                               ...
                               size_t name_buffer_length)
{
  ...
  case MTIDL_PROVIDER_WLAN:
    {
      wave_strcopy(name_buffer, interface_name, name_buffer_length);
      return MTLK_ERR_OK;
    }
...
_mtlk_wss_transact_irb(mtlk_irba_t *irba, mtlk_wss_data_source_t info_source,
                       uint32 info_id, uint16 info_index, void* data_buff, uint32 buff_length)
{
  int res;
  ...
  request_buffer->info_index = info_index;
  request_buffer->processing_result = MTLK_ERR_NO_ENTRY;

  wave_memcpy(&request_buffer[1], buff_length, data_buff, buff_length);
  ...
  if(MTLK_ERR_OK == res)
    wave_memcpy(data_buff, buff_length, &request_buffer[1], buff_length);
...
mtlk_wss_request_info(const char* interface_name, mtlk_wss_provider_level_t level,
                      mtlk_wss_data_source_t info_source, const void* provider_id,
                      uint32 info_id, uint16 info_index,
                      void* data_buff, uint32 buff_length)
{
  ...
  res = _mtlk_wss_transact_irb(target_node_irba, info_source, info_id,
                               info_index, data_buff, buff_length);
                   ...
                   mtlk_handle_t* wss_cleanup_context)
{
  ...
  if(MTLK_ERR_OK == res)
    res = mtlk_irba_app_create_stop_pipe();
...
mtlk_wssa_api_cleanup(mtlk_handle_t wss_cleanup_context)
{
  ...
  mtlk_irba_app_close_pipe_handles();
...
mtlk_wss_get_providers_list(const char* interface_name, mtlk_wss_provider_level_t level, int *res)
{
  ...
  case MTIDL_PROVIDER_HW:
    {
      res_list = mtlk_osal_mem_alloc(sizeof(mtlk_wss_provider_list_t) + _MAX_NODE_NAME_LENGTH, MTLK_MEM_TAG_WSS);
      if(NULL != res_list)
      {
  ...
  case MTIDL_PROVIDER_PEER:
    {
      ...
      if(NULL != wlan_node)
      {
        ...
        {
          res_list = mtlk_osal_mem_alloc(sizeof(mtlk_wss_provider_list_t) +
                                         (sizeof(IEEE_ADDR) + sizeof(BOOL)) * _MAX_PEERS_NUMBER, MTLK_MEM_TAG_WSS);
          if(NULL != res_list)
          {
            BOOL* sta_flags_array = (BOOL *) &((IEEE_ADDR *)res_list->provider_id_array)[_MAX_PEERS_NUMBER];

            res_list->providers_number = _mtlk_wss_fill_stations_array(wlan_node,
                    (IEEE_ADDR *)res_list->provider_id_array, sta_flags_array, _MAX_PEERS_NUMBER);

            if(res_list->providers_number < _MAX_PEERS_NUMBER)
            {
              memmove(&((IEEE_ADDR *)res_list->provider_id_array)[res_list->providers_number],
                      sta_flags_array, (sizeof(BOOL) * res_list->providers_number));
            }
            *res = MTLK_ERR_OK;
          }
          else
