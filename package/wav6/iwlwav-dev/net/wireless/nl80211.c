					 ...
					 struct cfg80211_bitrate_mask *mask)
{
	...
	nla_for_each_nested(tx_rates, info->attrs[NL80211_ATTR_TX_RATES], rem) {
		...
		err = nla_parse_nested(tb, NL80211_TXRATE_MAX, tx_rates,
				       nl80211_txattr_policy, genl_info_extack(info));
		if (err)
			return err;
				    ...
				    struct station_parameters *params)
{
	...
	if (info->attrs[NL80211_ATTR_HE_OPERATION])
		params->he_oper_iwlwav = nla_data(info->attrs[NL80211_ATTR_HE_OPERATION]);
...
static int nl80211_set_cqm(struct sk_buff *skb, struct genl_info *info)
{
	...
	err = nla_parse_nested(attrs, NL80211_ATTR_CQM_MAX, cqm,
			       nl80211_attr_cqm_policy, genl_info_extack(info));
	if (err)
		return err;
				...
				struct genl_info *info)
{
	...
	err = nla_parse_nested(tb, NL80211_NAN_FUNC_ATTR_MAX,
			       ...
			       nl80211_nan_func_policy, genl_info_extack(info));
	if (err)
		return err;
	...
	if (tb[NL80211_NAN_FUNC_SRF]) {
		...
		err = nla_parse_nested(srf_tb, NL80211_NAN_SRF_ATTR_MAX,
				       ...
				       nl80211_nan_srf_policy, genl_info_extack(info));
		if (err)
			goto out;
...
int nl80211_send_mgmt(struct cfg80211_registered_device *rdev,
		      ...
		      int freq, int sig_dbm, int snr_db,
		      const u8 *buf, size_t len, u32 flags, gfp_t gfp)
{
	...
	if (nla_put_u32(msg, NL80211_ATTR_WIPHY, rdev->wiphy_idx) ||
	    ...
	    (sig_dbm &&
	     nla_put_u32(msg, NL80211_ATTR_RX_SIGNAL_DBM, sig_dbm)) ||
	    (snr_db &&
	     nla_put_u32(msg, NL80211_ATTR_RX_SNR_DB, snr_db)) ||
	    nla_put(msg, NL80211_ATTR_FRAME, len, buf) ||
	    (flags &&
