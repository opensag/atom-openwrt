...
TRACE_EVENT(cfg80211_rx_mgmt,
	TP_PROTO(struct wireless_dev *wdev, int freq, int sig_dbm, int snr_db),
	TP_ARGS(wdev, freq, sig_dbm, snr_db),
	TP_STRUCT__entry(
		WDEV_ENTRY
		__field(int, freq)
		__field(int, sig_dbm)
	...
	TP_fast_assign(
		...
		__entry->snr_db = snr_db;
	),
	TP_printk(WDEV_PR_FMT ", freq: %d, sig dbm: %d, snr_db: %d",
		  WDEV_PR_ARG, __entry->freq, __entry->sig_dbm, __entry->snr_db)
);
