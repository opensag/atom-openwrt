...
typedef struct _mtlk_wss_provider_list_t
{
  ...
  char   provider_id_array[0]; /* < IDs array */
...
mtlk_wssa_access_byte_value(uint8 raw_value, mtlk_wss_data_source_t src)
{
  return raw_value;
}

static __INLINE uint16
mtlk_wssa_access_short_value(uint16 raw_value, mtlk_wss_data_source_t src)
{
  return (MTIDL_SRC_FW == src) ? MAC_TO_HOST16(raw_value) : raw_value;
}
