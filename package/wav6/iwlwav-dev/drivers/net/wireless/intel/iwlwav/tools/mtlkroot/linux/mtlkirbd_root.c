...
_mtlk_irbd_change_unique_desc (mtlk_irbd_t *irbd,
                               const char  *unique_desc)
{
  int len = strlen(unique_desc);
  void *buf = NULL;

  if (len != strlen(irbd->unique_desc)) {
    buf = kmalloc_tag(len + 1, GFP_KERNEL, MTLK_MEM_TAG_IRB_DESC);
    if (NULL == buf)
...
int __MTLK_IFUNC
mtlk_irbd_change_unique_desc (mtlk_irbd_t *irbd,
                              const char  *unique_desc)
{
  MTLK_ASSERT(irbd        != NULL);
  MTLK_ASSERT(unique_desc != NULL);

  return (_mtlk_irbd_change_unique_desc(irbd, unique_desc));
}
...
EXPORT_SYMBOL(mtlk_irbd_change_unique_desc);
