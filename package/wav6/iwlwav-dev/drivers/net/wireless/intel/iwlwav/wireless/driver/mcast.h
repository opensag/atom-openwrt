...
typedef struct _mcast_decref_sta_item {
  mtlk_slist_entry_t  list_entry;
  sta_entry           *sta;
} mcast_decref_sta_item;
...
int  mtlk_mc_parse (mtlk_core_t* nic, struct sk_buff *skb, sta_entry *sta)
...
int  mtlk_mc_transmit (mtlk_core_t* nic, mtlk_core_handle_tx_data_t *tx_data, uint32 nbuf_flags, int bridge_mode, sta_entry *src_sta)
  __MTLK_INT_HANDLER_SECTION;
void mtlk_mc_add_sta (mtlk_core_t *nic, sta_entry *sta);
void mtlk_mc_drop_sta (mtlk_core_t *nic, sta_entry *sta);
