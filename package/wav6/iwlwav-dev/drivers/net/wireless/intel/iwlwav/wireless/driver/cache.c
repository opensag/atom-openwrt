...
_cache_clone_country_ie(bss_data_t *bss)
{
  ...
  wave_memcpy(country_ie, ie_length, bss->country_ie, ie_length);
FINISH:
  return country_ie;
}
...
/*
 * NOTE: Currently this function is called from tasklet, therefore
 * no spin_lock_bh is required. If the protection of the critical section
 * is needed, call mtlk_cache_register_bss() function.
 */
void __MTLK_IFUNC
mtlk_cache_register_bss_nolock(scan_cache_t *cache, bss_data_t *bss)
{
  ...
  while ((next = mtlk_slist_next(prev)) != NULL) {
    ...
    if (!ieee_addr_compare(&place->bss.bssid, &bss->bssid)) {
      mtlk_slist_remove_next(&cache->bss_list, prev);
      slot = place;
      if (bss->essid[0] == '\0') {
        /* Preserve ESSID from active scan */
        wave_memcpy(bss->essid, sizeof(bss->essid), slot->bss.essid, sizeof(slot->bss.essid));
      }
      break;
    }
    prev = next;
  ...
  if (slot == NULL) {
    ...
    if (slot == NULL) {
      ILOG3_Y("Failed to register BSS %Y", bss->bssid.au8Addr);
      goto out;
    }
  ...
  /* place new BSS data sorted by channel, RSSI */
  prev = mtlk_slist_head(&cache->bss_list);
  while ((next = mtlk_slist_next(prev)) != NULL) {
    cache_entry_t *place = MTLK_LIST_GET_CONTAINING_RECORD(next, cache_entry_t, link_entry);
    ...
    /* Sort by RSSI */
    if (place->bss.channel == slot->bss.channel){
      if (place->bss.max_rssi < slot->bss.max_rssi){
        break;
      }
    ...
    if (place->bss.channel > slot->bss.channel){
      break;  /* first BSS on that channel. Skip RSSI sorting */
    }
  }

  mtlk_slist_insert_next(&cache->bss_list, prev, &slot->link_entry);
  ILOG3_Y("BSS %Y registered/updated", slot->bss.bssid.au8Addr);
