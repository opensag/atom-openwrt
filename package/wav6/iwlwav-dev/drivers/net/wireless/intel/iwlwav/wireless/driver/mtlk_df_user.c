...
mtlk_df_user_get_ndev_ops(mtlk_df_user_t *df_user)
{
  MTLK_ASSERT(NULL != df_user);

  return &df_user->dev_ops;
}
...
mtlk_df_user_t *
mtlk_df_user_ieee80211_ndev_to_dfuser (struct net_device *ndev)
{
...
mtlk_df_ui_set_mac_addr(mtlk_df_t *df, const uint8* mac_addr)
{
  ...
  mtlk_osal_copy_eth_addresses(mtlk_df_get_user(df)->ndev->dev_addr, mac_addr);
}
...
mtlk_df_ui_get_mac_addr(mtlk_df_t* df)
{
  ...
  return mtlk_df_get_user(df)->ndev->dev_addr;
}
...
/* FIXME: register netdev on init, but unregister on stop */
/*  For this we need REGISTER_DEVICE in both INIT and START */
#define _REG_NETDEV_WORKAROUND_     TRUE
...
MTLK_INIT_STEPS_LIST_BEGIN(df_user)
  ...
  MTLK_INIT_STEPS_LIST_ENTRY(df_user, NETDEV_INIT)
...
#if _REG_NETDEV_WORKAROUND_
  /* WAVE600: TODO: moved from start. Check this approach later */
  MTLK_INIT_STEPS_LIST_ENTRY(df_user, REGISTER_DEVICE)
#endif
#if MTLK_USE_PUMA6_UDMA
...
{
  mtlk_df_user_t *df_user = mtlk_df_user_from_skb(skb);
  if (NULL == df_user) {
...
_mtlk_df_user_priv_subxface_start (mtlk_df_user_t* df_user)
{
  ...
  if (mtlk_mmb_dcdp_path_available(hw)) {
    ...
    switch (dp_mode) {
      case MTLK_DC_DP_MODE_FASTPATH_GRX350:
      case MTLK_DC_DP_MODE_FASTPATH_GRX750:
...
static enum nl80211_iftype _get_iftype_by_role (mtlk_df_user_t *df_user)
{
  enum nl80211_iftype iftype;

  if (mtlk_vap_is_ap(mtlk_df_get_vap_handle(df_user->df)))
...
/* Relevant only for AP role interface */
static int
_mtlk_df_user_iface_open(struct net_device *ndev)
{
  int res = MTLK_ERR_OK;
  ...
  if (mtlk_core_rcvry_is_running(mtlk_vap_get_core(mtlk_df_get_vap_handle(df_user->df)))) {
    WLOG_V("Recovery is running. Try again");
    return -EAGAIN;
  }

  ILOG0_S("%s: start the interface", mtlk_df_user_get_name(df_user));
  ...
  iftype = _get_iftype_by_role(df_user);
  if (df_user->wdev.iftype != iftype) {
    WLOG_V("Wireless interface type change is not supported");
    return -ENOTSUPP;
  }
...
/* Relevant only for AP role interface */
static int
_mtlk_df_user_iface_stop(struct net_device *ndev)
{
  mtlk_hw_t *hw;
  mtlk_df_user_t* df_user =  mtlk_df_user_from_ndev(ndev);
  ...
  ILOG0_SS("%s, %s: stop the interface", current->comm, mtlk_df_user_get_name(df_user));
  ...
  if (!wave_rcvry_pci_probe_error_get(wave_hw_mmb_get_mmb_base(hw)))
  {
    ...
    */
    do {
      res = _mtlk_df_user_invoke_core(df_user->df, WAVE_CORE_REQ_DEACTIVATE, &clpb, NULL, 0);
...
/* Used only for AP role */
int __MTLK_IFUNC
mtlk_df_ui_linux_ioctl_setap (struct net_device *ndev,
                              ...
                              char *extra)
{
  ...
  ILOG1_SY("%s: Handle request: connect to %Y", ndev->name, sa->sa_data);

  res = _mtlk_df_user_invoke_core(df_user->df, WAVE_CORE_REQ_CONNECT_STA,
                                  &clpb, &sa->sa_data, sizeof(sa->sa_data));
  res = _mtlk_df_user_process_core_retval_void(res, clpb, WAVE_CORE_REQ_CONNECT_STA, TRUE);
...
/* Used only for AP role */
int __MTLK_IFUNC
mtlk_df_ui_linux_ioctl_setmlme (struct net_device *ndev,
                                struct iw_request_info *info,
                                ...
                                char *extra)
{
  ...
  if (!netif_running(ndev)) {
    if(mtlk_osal_eth_is_broadcast(mlme->addr.sa_data)){
  ...
  memset(&mlme_cfg, 0, sizeof(mlme_cfg));

  mlme_cfg.cmd = mlme->cmd;
  mlme_cfg.reason_code = mlme->reason_code;
  res = _mtlk_df_user_fill_ether_address(&mlme_cfg.sta_addr, &mlme->addr);
  if (MTLK_ERR_OK != res) {
    WLOG_SY("%s: Invalid address (%Y)", ndev->name, mlme->addr.sa_data);
    goto finish;
  }

  ILOG2_DD("MLME cmd %i reason_code %i", mlme->cmd, mlme->reason_code);

  switch (mlme->cmd) {
  case IW_MLME_DEAUTH:
  case IW_MLME_DISASSOC:
    ILOG1_SY("%s: Got MLME Disconnect/Disassociate ioctl (%Y)", ndev->name, mlme->addr.sa_data);
    if (!mtlk_df_is_ap(df_user->df)) {
#ifdef MTLK_LEGACY_STATISTICS
      res = _mtlk_df_user_invoke_core(df_user->df, WAVE_CORE_REQ_DISCONNECT_STA,
                                      &clpb, (char*)&mlme_cfg, sizeof(mlme_cfg));
      res = _mtlk_df_user_process_core_retval_void(res, clpb, WAVE_CORE_REQ_DISCONNECT_STA, TRUE);
#endif
    }
    else if(mtlk_osal_eth_is_broadcast(mlme->addr.sa_data)) {
      res = _mtlk_df_user_invoke_core(df_user->df, WAVE_CORE_REQ_AP_DISCONNECT_ALL,
                                      &clpb, (char*)&mlme_cfg, sizeof(mlme_cfg));
      ...
      ILOG1_SY("%s: Invalid MAC address (%Y)!", ndev->name, mlme->addr.sa_data);
      res = MTLK_ERR_PARAMS;
      goto finish;
    }
    else {
      res = _mtlk_df_user_invoke_core(df_user->df, WAVE_CORE_REQ_AP_DISCONNECT_STA,
                                      &clpb, (char*)&mlme_cfg, sizeof(mlme_cfg));
      res = _mtlk_df_user_process_core_retval_void(res, clpb, WAVE_CORE_REQ_AP_DISCONNECT_STA, TRUE);
    }
    break;
  default:
...
static int
_mtlk_df_user_set_mac_addr (struct net_device *ndev, void* p)
{
  return _mtlk_df_user_set_mac_addr_internal(ndev, (struct sockaddr *)p);
}
...
static int
_mtlk_df_user_fill_fixed_pwr_params (uint32 *intvec, uint16 intvec_length, FixedPower_t *fixed_pwr_params)
{
  if (intvec_length != 4) {
...
_mtlk_df_user_fill_peer_ap_key_idx (uint8 *peer_ap_key_idx, uint32 data)
{
  ...
  if (data > MIB_WEP_N_DEFAULT_KEYS)
  {
    ELOG_DD("Wrong key index %d. Usage: <[1-%d]|0> (0 means Open)", data, MIB_WEP_N_DEFAULT_KEYS);
    return MTLK_ERR_VALUE;
  }
...
static int
_mtlk_df_user_irb_pinger_int_get_cfg (mtlk_df_user_t* df_user, uint32 subcmd, char* data, mtlk_iwpriv_params_t params, uint16* length)
{
  ...
  switch (subcmd) {
    ...
    case PRM_ID_IRB_PINGER_STATS:
      ...
      *length = _mtlk_df_user_print_irb_pinger_stats(data, MTLK_IW_PRIV_LENGTH(params), &stats);
      return MTLK_ERR_OK;
    default:
...
static int
_mtlk_df_user_irb_pinger_int_set_cfg (mtlk_df_user_t* df_user, uint32 subcmd, char* data, mtlk_iwpriv_params_t params)
{
  MTLK_ASSERT(df_user != NULL);
  switch (subcmd) {
    case PRM_ID_IRB_PINGER_ENABLED:
...
static int
_mtlk_df_user_get_interfdet_req_scantimes_by_intvec (const uint32 *intvec, uint16 intvec_length, mtlk_interfdet_req_scantimes_t *req_params)
{
  int res = MTLK_ERR_OK;
  int i;

  if (2 != intvec_length) {
    ELOG_D("Incorrect vector length. length(%u), expected 2", intvec_length);
    res = MTLK_ERR_PARAMS;
  } else for (i = 0; i < 2; i++) {
    if ((int32)intvec[i] < 0) {
      ELOG_DD("Incorrect parameter %d (%d)", i, intvec[i]);
      res = MTLK_ERR_PARAMS;
      break;
    }
  }
  ...
  if (MTLK_ERR_PARAMS == res) {
    ELOG_V("Expected parameters: <short_scan_max_time> <long_scan_max_time>");
    return res;
  }

  req_params->short_scan_max_time = intvec[0];
  req_params->long_scan_max_time  = intvec[1];
  return res;
}

static int
_mtlk_df_user_get_interfdet_req_timeouts_by_intvec (const uint32 *intvec, uint16 intvec_length, mtlk_interfdet_req_timeouts_t *req_params)
{
  int res = MTLK_ERR_OK;

  if (6 != intvec_length) {
    ELOG_D("Incorrect vector length. length(%u), expected 6", intvec_length);
    res = MTLK_ERR_PARAMS;
  }

  if (MTLK_ERR_PARAMS == res) {
    ELOG_V("Expected parameters: <active_polling_timeout> <short_scan_polling_timeout> <long_scan_polling_timeout> "\
                  "<active_notification_timeout> <short_scan_notification_timeout> <long_scan_notification_timeout>");
    return res;
  }

  req_params->active_polling_timeout          = intvec[0];
  req_params->short_scan_polling_timeout      = intvec[1];
  req_params->long_scan_polling_timeout       = intvec[2];
  req_params->active_notification_timeout     = intvec[3];
  req_params->short_scan_notification_timeout = intvec[4];
  req_params->long_scan_notification_timeout  = intvec[5];
...
_mtlk_df_user_get_coc_antenna_params_by_intvec (const uint32 *intvec, uint16 intvec_length, mtlk_coc_antenna_cfg_t *antenna_params)
{
  ...
  if ((intvec[0] == 1) && (intvec_length == 1))
	return MTLK_ERR_OK;
  else if (((intvec[0] == 1) && (intvec_length != 1)) ||
      ((intvec[0] == 0) && (intvec_length != 3))) {
    ELOG_DD("Incorrect vector configuration: <enable mode %u> and length %u", intvec[0], intvec_length);
  }
  else if ((intvec[0] != 1) && (intvec[0] != 0)) {
    ELOG_D("Incorrect vector configuration: <enable mode %u>", intvec[0]);
  }
  else if ((intvec[0] == 0) && ((intvec[1] == 0) || (intvec[2] == 0))) {
    ELOG_D("Incorrect vector configuration: <enable mode %u> and zero antennas", intvec[0]);
  }
  else {
    antenna_params->num_tx_antennas = intvec[1];
    antenna_params->num_rx_antennas = intvec[2];
...
#define MTLK_DF_USER_TXOP_MODE_PARAM_COUNT 2

static int
_mtlk_df_user_get_txop_mode_params_by_intvec(const uint32 *intvec, uint16 intvec_length, mtlk_core_txop_mode_t *txop_mode)
{
  if (intvec_length != MTLK_DF_USER_TXOP_MODE_PARAM_COUNT) {
    ELOG_DD("Incorrect vector length (%u), must be %u", intvec_length, MTLK_DF_USER_TXOP_MODE_PARAM_COUNT);
    ELOG_V("Usage: <SID> <TXOP mode>");
    return MTLK_ERR_PARAMS;
  }

  txop_mode->sid  = intvec[0];
  txop_mode->mode = intvec[1];
...
static void
_mtlk_df_user_set_intvec_by_txop_mode_params(uint32 *intvec, uint16 max_length, uint16 *intvec_length, const mtlk_core_txop_mode_t *txop_mode)
{
  MTLK_ASSERT(max_length >= MTLK_DF_USER_TXOP_MODE_PARAM_COUNT);

  if (/* (ALL_STA_SID == txop_mode->sid) && */ /* FIXME: HW dependent */
      (MTLK_PARAM_DB_VALUE_IS_INVALID(txop_mode->mode))) {
    ELOG_V("Fixed Power not Set, Getter invalid!");
    *intvec_length = 0;
    return;
  }

  intvec[0] = txop_mode->sid;
  intvec[1] = txop_mode->mode;
  *intvec_length = MTLK_DF_USER_TXOP_MODE_PARAM_COUNT;
}
...
static int
_mtlk_df_user_get_calib_cw_masks_by_intvec(const uint32 *intvec, uint32 intvec_length,
                                           uint32 calib_cw_masks[NUM_IWPRIV_CALIB_CW_MASKS])
{
  int res = MTLK_ERR_PARAMS;

  if (intvec_length != NUM_IWPRIV_CALIB_CW_MASKS)
  {
    ELOG_DD("%i parameters given but %i are needed", intvec_length, NUM_IWPRIV_CALIB_CW_MASKS);
  }
  else
  {
    calib_cw_masks[MTLK_HW_BAND_2_4_GHZ] = intvec[0];
    calib_cw_masks[MTLK_HW_BAND_5_2_GHZ] = intvec[1];
...
{
  MTLK_STATIC_ASSERT(6 == NUM_IWPRIV_SCAN_PARAMS_BG);
...
static void
_mtlk_df_user_get_intcev_by_calib_cw_masks(uint32 *intvec, uint16 max_length, uint16 *intvec_length,
                                           const uint32 calib_cw_masks[NUM_IWPRIV_CALIB_CW_MASKS])
{
  MTLK_ASSERT(max_length >= NUM_IWPRIV_CALIB_CW_MASKS);

  intvec[0] = calib_cw_masks[MTLK_HW_BAND_2_4_GHZ];
  intvec[1] = calib_cw_masks[MTLK_HW_BAND_5_2_GHZ];
  *intvec_length = NUM_IWPRIV_CALIB_CW_MASKS;
}
...
static void
_mtlk_df_user_get_intvec_by_fixed_pwr_params (uint32 *intvec, uint16 max_length, uint16 *intvec_length, FixedPower_t *fixed_pwr_params)
{
  MTLK_ASSERT(max_length >= 4);
...
_mtlk_df_user_fill_ht_protection_cfg(mtlk_df_user_t *df_user, mtlk_ht_protection_cfg_t *cfg, uint32 data)
{
    ...
    BOOL is_gen5 = mtlk_hw_type_is_gen5(hw);
    BOOL is_gen6 = mtlk_hw_type_is_gen6(hw);

    if ((data == PROTECTION_MODE_NO_PROTECTION) ||
        (data == PROTECTION_MODE_NO_FORCE_PROTECTION) ||
        (data == PROTECTION_MODE_FORCE_RTS_CTS) ||
        ((data == PROTECTION_MODE_FORCE_CTS_TO_SELF) && (is_gen5 | is_gen6)))
    {
...
static int
_mtlk_df_user_do_simple_cli (const uint32 *intvec, uint16 intvec_length, UmiDbgCliReq_t *DbgCliReq)
{
  int res = MTLK_ERR_PARAMS;
  ...
  if ((1 > intvec_length) || (4 < intvec_length)) {
    ...
    memset(DbgCliReq, 0, sizeof(UmiDbgCliReq_t));
...
static int
_mtlk_df_user_fill_fixed_ltf_and_gi_by_intvec(uint32 *intvec, uint16 intvec_length, UMI_FIXED_LTF_AND_GI_REQ_t *fixed_ltf_and_gi)
{
    if (intvec_length != 2) {
      ELOG_D("Need exactly 2 arguments (length=%u)", intvec_length);
      return MTLK_ERR_PARAMS;
...
static void
_mtlk_df_user_get_intvec_by_fixed_ltf_and_gi (uint32 *intvec, uint16 max_length, uint16 *intvec_length, const UMI_FIXED_LTF_AND_GI_REQ_t *fixed_ltf_and_gi)
{
    MTLK_ASSERT(max_length >= 2);
...
_mtlk_df_user_iwpriv_get_param(mtlk_df_user_t* df_user, uint32 param_id, char* data, mtlk_iwpriv_params_t params, uint16* length)
{
  ...
  _DF_USER_GET_PARAM_MAP_START(df_user, param_id, res)
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_USE_SHORT_CYCLIC_PREFIX, WAVE_RADIO_REQ_GET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      MTLK_CFG_GET_ITEM(mibs_cfg, short_cyclic_prefix, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(MIB_LONG_RETRY_LIMIT, WAVE_RADIO_REQ_GET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      MTLK_CFG_GET_ITEM(mibs_cfg, long_retry_limit, *(uint32*)data);

    _DF_USER_GET_ON_PARAM_MEMBER(PRM_ID_HIDDEN_SSID, WAVE_CORE_REQ_GET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, core_cfg, is_hidden_ssid)
      MTLK_CFG_GET_ITEM(core_cfg, is_hidden_ssid, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_COUNTRY, WAVE_RADIO_REQ_GET_COUNTRY_CFG, FALSE, mtlk_country_cfg_t, country_cfg)
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(country_cfg, country_code, wave_strncopy, /* result is a NUL-terminated string */
                                      (data, country_cfg->country_code.country,
                                       max_length, sizeof(country_cfg->country_code.country)));
      *length = sizeof(country_cfg->country_code.country);

    _DF_USER_GET_ON_PARAM(PRM_ID_11D, WAVE_RADIO_REQ_GET_DOT11D_CFG, FALSE, mtlk_dot11d_cfg_t, dot11d_cfg)
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_STA_KEEPALIVE_INTERVAL, WAVE_CORE_REQ_GET_STADB_CFG, FALSE, mtlk_stadb_cfg_t, stadb_cfg)
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_HE_MU_OPERATION, WAVE_RADIO_REQ_GET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_cfg)
    ...
    _DF_USER_GET_ON_PARAM_MEMBER(PRM_ID_BSS_LOAD_IN_BEACON, WAVE_CORE_REQ_GET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, core_cfg, is_bss_load_enable)
      MTLK_CFG_GET_ITEM(core_cfg, is_bss_load_enable, *(uint32*)data);
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_CALIB_CW_MASKS, WAVE_RADIO_REQ_GET_SCAN_AND_CALIB_CFG, FALSE, mtlk_scan_and_calib_cfg_t, scan_and_calib_cfg)
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(scan_and_calib_cfg, calib_cw_masks, _mtlk_df_user_get_intcev_by_calib_cw_masks,
                                     ((uint32*)data, max_length, length, scan_and_calib_cfg->calib_cw_masks));
    ...
    /* TXOP mode configuration processing */
    _DF_USER_GET_ON_PARAM_MEMBER(PRM_ID_TXOP_CONFIG, WAVE_RADIO_REQ_GET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_cfg, txop_mode_params)
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(master_cfg, txop_mode_params, _mtlk_df_user_set_intvec_by_txop_mode_params,
                                     ((uint32*)data, max_length, length, &master_cfg->txop_mode_params));
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_COEX_CFG, WAVE_RADIO_REQ_GET_COEX_CFG, FALSE, mtlk_coex_cfg_t, coex_cfg)
      MTLK_CFG_GET_ITEM(coex_cfg, coex_mode, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_COEX_ENABLE, WAVE_RADIO_REQ_GET_COEX_CFG, FALSE, mtlk_coex_cfg_t, coex_cfg)
      MTLK_CFG_GET_ITEM(coex_cfg, coex_enable, *(uint32*)data);
    ...
    _DF_USER_GET_ON_PARAM_MEMBER(PRM_ID_UP_RESCAN_EXEMPTION_TIME, WAVE_CORE_REQ_GET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, core_cfg, up_rescan_exemption_time)
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_CDB_CFG, WAVE_RADIO_REQ_GET_CDB_CFG, FALSE, wave_core_cdb_cfg_t, cdb)
      MTLK_CFG_GET_ITEM(cdb, cdb_cfg, *(uint32*)data);

    _DF_USER_GET_ON_PARAM_MEMBER(PRM_ID_BSS_BASIC_RATE_SET, WAVE_CORE_REQ_GET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, core_cfg, bss_rate)
      MTLK_CFG_GET_ITEM(core_cfg, bss_rate, *(uint32*)data);
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_HSTDB_LOCAL_MAC, WAVE_CORE_REQ_GET_HSTDB_CFG, FALSE, mtlk_hstdb_cfg_t, hstdb_cfg)
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(hstdb_cfg, address, _mtlk_df_user_fill_sockaddr,
                                     ((struct sockaddr*)data, &hstdb_cfg->address));
    ...
    /* Four address configuration processing */
    _DF_USER_GET_ON_PARAM(PRM_ID_4ADDR_MODE, WAVE_CORE_REQ_GET_FOUR_ADDR_CFG, FALSE, mtlk_four_addr_cfg_t, four_addr_cfg)
      MTLK_CFG_GET_ITEM(four_addr_cfg, mode, *(uint32*)data);
...
_mtlk_df_user_iwpriv_set_param(mtlk_df_user_t* df_user, uint32 param_id, char* data, mtlk_iwpriv_params_t params)
{
  ...
  _DF_USER_SET_PARAM_MAP_START(df_user, param_id, res)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_USE_SHORT_CYCLIC_PREFIX, WAVE_RADIO_REQ_SET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      MTLK_CFG_SET_ITEM(mibs_cfg, short_cyclic_prefix, *(uint32*)data);

    _DF_USER_SET_ON_PARAM(MIB_LONG_RETRY_LIMIT, WAVE_RADIO_REQ_SET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      MTLK_CFG_SET_ITEM(mibs_cfg, long_retry_limit, *(uint32*)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_HIDDEN_SSID, WAVE_CORE_REQ_SET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, core_cfg)
      MTLK_CFG_SET_ITEM(core_cfg, is_hidden_ssid, (*(uint32*)data)?TRUE:FALSE);

    _DF_USER_SET_ON_PARAM(PRM_ID_COUNTRY, WAVE_RADIO_REQ_SET_COUNTRY_CFG, FALSE, mtlk_country_cfg_t, country_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC_VOID(country_cfg, country_code, wave_strcopy,
                                     (country_cfg->country_code.country, data,
                                     (sizeof(country_cfg->country_code.country))));
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_11D, WAVE_RADIO_REQ_SET_DOT11D_CFG, FALSE, mtlk_dot11d_cfg_t, dot11d_cfg)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_STA_KEEPALIVE_INTERVAL, WAVE_CORE_REQ_SET_STADB_CFG, FALSE, mtlk_stadb_cfg_t, stadb_cfg)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_UP_RESCAN_EXEMPTION_TIME, WAVE_CORE_REQ_SET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, pcore_cfg)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_BSS_BASIC_RATE_SET, WAVE_CORE_REQ_SET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, pcore_cfg)
      MTLK_CFG_SET_ITEM(pcore_cfg, bss_rate, *(uint32*)data);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_ESSID, WAVE_CORE_REQ_SET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, pcore_cfg)
      MTLK_CFG_SET_ARRAY_ITEM_BY_FUNC_VOID(pcore_cfg, essid, wave_strncopy,
                                            (pcore_cfg->essid, data, sizeof(pcore_cfg->essid), length));

    _DF_USER_SET_ON_PARAM(PRM_ID_BSS_LOAD_IN_BEACON, WAVE_CORE_REQ_SET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, pcore_cfg)
      MTLK_CFG_SET_ITEM(pcore_cfg, is_bss_load_enable, (*(uint32*)data) ? TRUE : FALSE);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_HSTDB_LOCAL_MAC, WAVE_CORE_REQ_SET_HSTDB_CFG, FALSE, mtlk_hstdb_cfg_t, hstdb_cfg)
    ...
    /* Interference Detection */
    _DF_USER_SET_ON_PARAM(PRM_ID_INTERFER_TIMEOUTS, WAVE_RADIO_REQ_SET_INTERFDET_PARAMS_CFG, FALSE, mtlk_interfdet_cfg_t, interfdet_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(interfdet_cfg, req_timeouts,
                                _mtlk_df_user_get_interfdet_req_timeouts_by_intvec, ((uint32*)data, length, &interfdet_cfg->req_timeouts), res);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_INTERFER_SCANTIMES, WAVE_RADIO_REQ_SET_INTERFDET_PARAMS_CFG, FALSE, mtlk_interfdet_cfg_t, interfdet_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(interfdet_cfg, req_scantimes,
                                _mtlk_df_user_get_interfdet_req_scantimes_by_intvec, ((uint32*)data, length, &interfdet_cfg->req_scantimes), res);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_CALIB_CW_MASKS, WAVE_RADIO_REQ_SET_SCAN_AND_CALIB_CFG, FALSE, mtlk_scan_and_calib_cfg_t, scan_and_calib_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(scan_and_calib_cfg, calib_cw_masks,
                                _mtlk_df_user_get_calib_cw_masks_by_intvec, ((uint32*)data, length, scan_and_calib_cfg->calib_cw_masks), res);
   ...
   _DF_USER_SET_ON_PARAM(PRM_ID_CPU_DMA_LATENCY, WAVE_RADIO_REQ_SET_CPU_DMA_LATENCY, FALSE, mtlk_pm_qos_cfg_t, pm_qos_cfg)
    ...
    /* Four address configuration processing */
    _DF_USER_SET_ON_PARAM(PRM_ID_4ADDR_MODE, WAVE_CORE_REQ_SET_FOUR_ADDR_CFG, FALSE, mtlk_four_addr_cfg_t, four_addr_cfg)
      MTLK_CFG_SET_ITEM(four_addr_cfg, mode, *(uint32*)data);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_TXOP_CONFIG, WAVE_RADIO_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_core_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC_VOID(master_core_cfg, txop_mode_params, _mtlk_df_user_get_txop_mode_params_by_intvec,
        ((uint32*)data, length, &master_core_cfg->txop_mode_params));
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_COEX_CFG, WAVE_RADIO_REQ_SET_COEX_CFG, FALSE, mtlk_coex_cfg_t, coex_cfg)
      MTLK_CFG_SET_ITEM(coex_cfg, coex_mode, *(uint32*)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_COEX_ENABLE, WAVE_RADIO_REQ_SET_COEX_CFG, FALSE, mtlk_coex_cfg_t, coex_cfg)
      MTLK_CFG_SET_ITEM(coex_cfg, coex_enable, *(uint32*)data);
...
_mtlk_ioctl_drv_gen_data_exchange (mtlk_df_user_t* df_user, struct ifreq *ifr)
{
  ...
  res = _mtlk_df_user_invoke_core(df_user->df,
                                 ...
                                 &clpb, &req, sizeof(req));
  res = _mtlk_df_user_process_core_retval_void(res, clpb, WAVE_CORE_REQ_GEN_DATA_EXCHANGE, FALSE);
  ...
  if (0 != (copy_to_user(ifr->ifr_data, &resp, sizeof(resp)))) {
    res = MTLK_ERR_VALUE;
  }
...
static int
mtlk_df_do_command (struct net_device *ndev, struct ifreq *ifr, int cmd)
{
  mtlk_df_user_t* df_user = mtlk_df_user_from_ndev(ndev);
  int res = -EOPNOTSUPP;

  ILOG3_SSDD("%s: Invoked from %s (%i), cmd is 0x%04x", ndev->name, current->comm, current->pid, cmd);
  MTLK_CHECK_DF_USER(df_user);
...
static int
_mtlk_df_user_change_mtu(struct net_device *ndev, int new_mtu)
{
  int res = MTLK_ERR_OK;

  if ((new_mtu < MTLK_MIN_MTU_SIZE) || (new_mtu > (RX_DATA_FRAME_SIZE - MAX_FW_RX_DATA_OFFSET)))
    res = MTLK_ERR_PARAMS;
  else
    ndev->mtu = new_mtu;
...
static int
_mtlk_df_change_carrier (struct net_device *ndev, bool new_carrier)
{
  int res = MTLK_ERR_OK;

  if (FALSE == new_carrier) {
    if (netif_carrier_ok(ndev))
      netif_carrier_off(ndev);
  }
  else if (TRUE == new_carrier) {
    if (!netif_carrier_ok(ndev))
...
static void _mtlk_df_user_fill_callbacks(mtlk_df_user_t *df_user)
{
  struct net_device_ops dev_ops = {
    ...
    .ndo_open            = _mtlk_df_user_iface_open,
    .ndo_stop            = _mtlk_df_user_iface_stop,
    .ndo_set_mac_address = _mtlk_df_user_set_mac_addr,
#if LINUX_VERSION_IS_GEQ(4,11,0)
    .ndo_get_stats64     = _mtlk_df_user_get_stats,
#else
    .ndo_get_stats64 = bp__mtlk_df_user_get_stats,
#endif
    .ndo_get_stats       = NULL,
    .ndo_tx_timeout      = NULL,
    .ndo_change_mtu      = _mtlk_df_user_change_mtu,
    .ndo_do_ioctl        = mtlk_df_do_command,
    .ndo_change_carrier  = _mtlk_df_change_carrier,
  };

  df_user->dev_ops = dev_ops;
  df_user->ndev->netdev_ops = &df_user->dev_ops;
}
...
/* WAVE600: TODO: moved to init. Check this approach later */
#if _REG_NETDEV_WORKAROUND_
...
static int
_mtlk_df_user_reg_netdev (mtlk_df_user_t *df_user)
{
  int res = 0;
  /* For station mode interfaces net device registration is performed by mac80211 */
  BOOL is_ap = mtlk_vap_is_ap(mtlk_df_get_vap_handle(df_user->df));
  ILOG1_SDD("%s: is_ap=%d, is_reg=%d", df_user->ndev->name, (int)is_ap, (int)df_user->ndev_registered);
  if (is_ap && !df_user->ndev_registered) {
    res = register_netdevice(df_user->ndev);
    if (0 != res) {
      ELOG_D("register_netdevice failed, err %d", res);
    } else {
...
static void
_mtlk_df_user_unreg_netdev (mtlk_df_user_t *df_user)
{
  /* For station mode interfaces net device unregistration is performed by mac80211 */
  BOOL is_ap = mtlk_vap_is_ap(mtlk_df_get_vap_handle(df_user->df));
  ILOG1_SDD("%s: is_ap=%d, is_reg=%d", df_user->ndev->name, (int)is_ap, (int)df_user->ndev_registered);
  if (is_ap && df_user->ndev_registered) {
    /* Can be called with rtnl locked already */
    int locked = rtnl_trylock();
    unregister_netdevice(df_user->ndev);
    if (locked) rtnl_unlock();
    df_user->ndev_registered = FALSE;
  }
}
...
mtlk_df_user_stop(mtlk_df_user_t *df_user)
{
  ...
  MTLK_STOP_BEGIN(df_user, MTLK_OBJ_PTR(df_user))
    ...
    /* WAVE600: TODO: moved to init. Check this approach later */
#if _REG_NETDEV_WORKAROUND_ /* register on init, unregister on stop */
    MTLK_STOP_STEP(df_user, REGISTER_DEVICE, MTLK_OBJ_PTR(df_user),
                   _mtlk_df_user_unreg_netdev, (df_user));
#endif
...
mtlk_df_user_start(mtlk_df_t *df, mtlk_df_user_t *df_user)
{
  ...
  MTLK_START_TRY(df_user, MTLK_OBJ_PTR(df_user))
    /* WAVE600: TODO: moved to init. Check this approach later */
#if _REG_NETDEV_WORKAROUND_ /* register on init, unregister on stop */
     MTLK_START_STEP_VOID(df_user, REGISTER_DEVICE, MTLK_OBJ_PTR(df_user),
                     MTLK_NOACTION, ());
#else /* register on start */
    MTLK_START_STEP(df_user, REGISTER_DEVICE, MTLK_OBJ_PTR(df_user),
                    _mtlk_df_user_reg_netdev, (df_user));
#endif

    hw = mtlk_vap_get_hw(mtlk_df_get_vap_handle(df));
    if (wave_hw_mmb_get_stats_poll_period(hw) == 0) {
      wave_hw_mmb_set_stats_poll_period(df, _DF_STAT_POLL_PERIOD);
    }
    MTLK_START_STEP(df_user, STAT_POLL, MTLK_OBJ_PTR(df_user),
                    mtlk_osal_timer_set,
                    (&df_user->slow_ctx->stat_timer, _DF_STAT_POLL_PERIOD));
    MTLK_START_STEP_VOID_IF(mtlk_vap_is_master(mtlk_df_get_vap_handle(df_user->df)),
...
static int df_ui_wds_dbg(mtlk_seq_entry_t *s, void *v)
{
  ...
  /*mtlk_aux_seq_printf(s," | 11:22:33:44:55:66 | 11:22:33:44:55:66 | 0x12345678 | 123...     \n");*/
    while (h) {
      ...
      mtlk_aux_seq_printf(s," | " MAC_PRINTF_FMT " | " MAC_PRINTF_FMT " | %p | %u\n",
                          MAC_PRINTF_ARG(addr->au8Addr),
...
static int mtlk_df_ui_tx_power(mtlk_seq_entry_t *s, void *data)
{
  ...
  mtlk_clpb_t *clpb = NULL;
  mtlk_df_t *df = mtlk_df_proc_seq_entry_get_df(s);
  char *min_power_interpretation = "Ultimate EVM Power"; /* for both gen5 and gen6 */
  ...
  while (NULL != (entry = mtlk_clpb_enum_get_next(clpb, NULL))) {
    ...
    /* In Gen5/Gen6 pw_min_ant is an Ultimate EVM Power, not MIN power */
    ...
    for (i = 0; i < pw_size; i++) {
      ...
      {
        char buf[64];
        snprintf(buf, 64, "Value: phy_mode:%-9s mcs:%3d nss:%3d",
...
void
mtlk_df_inc_corrupted_packets(mtlk_df_t *df)
{
  mtlk_df_user_t* df_user = mtlk_df_get_user(df);
  df_user->corrupted_packets_counter++;
}

static int mtlk_df_ui_corrupted_packet(mtlk_seq_entry_t *s, void *data)
{
  int res = MTLK_ERR_OK;
  mtlk_df_t *df = mtlk_df_proc_seq_entry_get_df(s);
  mtlk_df_user_t* df_user = mtlk_df_get_user(df);
  mtlk_aux_seq_printf(s, "corrupted packets:%d\n", df_user->corrupted_packets_counter);
...
static void _mtlk_df_ui_print_dcdp_stats (mtlk_seq_entry_t *s, mtlk_df_user_t *df_user)
{
    ...
    mtlk_df_user_t *df_user_master = mtlk_df_get_user(mtlk_df_user_get_master_df(df_user));
...
static int mtlk_df_ui_debug_hs20_info(mtlk_seq_entry_t *s, void *data)
{
  uint32 size;
  ...
  res = _mtlk_df_user_invoke_core(df, WAVE_CORE_REQ_GET_HS20_INFO, &clpb, NULL, 0);
  res = _mtlk_df_user_process_core_retval_void(res, clpb, WAVE_CORE_REQ_GET_HS20_INFO, FALSE);
  if (MTLK_ERR_OK != res) {
    goto err_ret;
  }

  hs20_info = mtlk_clpb_enum_get_next(clpb, &size);

  if (hs20_info && (sizeof(*hs20_info) == size)) {
    mtlk_aux_seq_printf(s, uint_fmt, hs20_info->hs20_enabled,  "HS20 enabled");
    mtlk_aux_seq_printf(s, uint_fmt, hs20_info->arp_proxy,     "ARP Proxy");
    mtlk_aux_seq_printf(s, uint_fmt, hs20_info->dgaf_disabled, "DGAF disabled");
    mtlk_aux_seq_printf(s, uint_fmt, hs20_info->osen_enabled,  "OSEN enabled");
  }
  else {
    res = MTLK_ERR_UNKNOWN;
  }
...
_mtlk_df_ui_ee_caps(mtlk_seq_entry_t *s, void *data)
{
  ...
  res = _mtlk_df_user_pull_core_data(df, WAVE_CORE_REQ_GET_EE_CAPS, TRUE, (void**) &pstat, NULL, &hdata);
  if (MTLK_ERR_OK != res) {
    goto err_ret;
  }
...
_mtlk_df_ui_scanrequest_read(mtlk_seq_entry_t *s, void *data)
{
  ...
  int nummatchsets = MIN(req->n_match_sets, MAX_MATCH_SETS);
  ...
  mtlk_aux_seq_printf(s, "n_match_sets:   %u\n", req->n_match_sets);
  ...
  mtlk_aux_seq_printf(s, "\n# match_ssid\trssi_thold\n");

  for (i = 0; i < nummatchsets; i++)
  {
    struct mtlk_match_set *ms = req->match_sets + i;
    mtlk_aux_seq_printf(s, "%.*s\t%i\n", ms->ssid_len, ms->ssid, ms->rssi_thold);
  }
...
static int _mtlk_df_qos_map_dump(mtlk_seq_entry_t *s, void *v)
{
  ...
  if (hs20_info && (sizeof(*hs20_info) == size)) {
    seq_file_hex_dump(s, hs20_info->dscp_table, sizeof(hs20_info->dscp_table));
  }
  else {
    res = MTLK_ERR_UNKNOWN;
  }
...
_mtlk_df_ui_eeprom(mtlk_seq_entry_t *s, void *data)
{
  ...
  if(MTLK_ERR_OK == res)
  {
    ...
    res = mtlk_aux_seq_write(s, eeprom_cfg->eeprom_raw_data, sizeof(eeprom_cfg->eeprom_raw_data));
  }
...
#define PROC_MHI_STATS "mhi_stats"

static int
_mtlk_df_ui_mhi_stats_show(mtlk_seq_entry_t *s, void *data)
{
  mtlk_df_t         *df = mtlk_df_proc_seq_entry_get_df(s);
  mtlk_clpb_t       *clpb = NULL;
  ...
  ILOG2_PDP("Buffer %p size %d, data ptr %p", s->buf, (int)s->size, data);

  buf_size = s->size - 1;   /* -1 to prevent buffer overflow */
  mhi_stats.mhi_stats_data = s->buf;
  mhi_stats.mhi_stats_size = buf_size;
  MTLK_ASSERT(buf_size > 0);

  res = _mtlk_df_user_invoke_core(df, WAVE_RADIO_REQ_GET_MHI_STATS, &clpb, &mhi_stats, sizeof(mhi_stats));
  res = _mtlk_df_user_process_core_retval_void(res, clpb, WAVE_RADIO_REQ_GET_MHI_STATS, TRUE);

  if (MTLK_ERR_OK == res) {
      mtlk_aux_seq_write(s, mhi_stats.mhi_stats_data, buf_size);
  }

  return _mtlk_df_mtlk_to_linux_error_code(res);
}

static int mtlk_df_ui_reg_mhi_stats_read(mtlk_df_user_t* df_user, unsigned size)
{
  /* only for master vap */
  if (mtlk_vap_is_master(mtlk_df_get_vap_handle(df_user->df))) {
    return mtlk_df_proc_node_add_seq_entry_size(PROC_MHI_STATS,
             df_user->slow_ctx->proc_df_node, df_user->df, _mtlk_df_ui_mhi_stats_show, size);
  } else {
    return MTLK_ERR_OK;
  }
}

static void mtlk_df_ui_unreg_mhi_stats_read(mtlk_df_user_t* df_user)
{
  /* only for master vap */
  if (mtlk_vap_is_master(mtlk_df_get_vap_handle(df_user->df))) {
    mtlk_df_proc_node_remove_entry(PROC_MHI_STATS, df_user->slow_ctx->proc_df_node);
  }
}

int __MTLK_IFUNC
mtlk_df_ui_mhi_stats_create(mtlk_df_user_t* df_user, unsigned size)
{
    return mtlk_df_ui_reg_mhi_stats_read(df_user, size);
}

void __MTLK_IFUNC
mtlk_df_ui_mhi_stats_delete(mtlk_df_user_t* df_user, unsigned size)
{
    mtlk_df_ui_unreg_mhi_stats_read(df_user);
}
...
#endif /* EEPROM_DATA_ACCESS */
  { "EECaps",           _NODBG_,  _mtlk_df_ui_ee_caps           },
  { "hw_limits",        _NODBG_,  mtlk_df_ui_hw_limits          },
  { "tx_power",         _NODBG_,  mtlk_df_ui_tx_power           },
  { "igmp",             _NODBG_,  _mtlk_df_ui_debug_igmp_read   },
  { "channel",          _NODBG_,  _mtlk_df_ui_channel_read      },
  { "hdk_config",       _NODBG_,  _mtlk_df_ui_hdkconfig_read    },
  { "calibration",      _NODBG_,  _mtlk_df_ui_calibration_read  },
  { "scan_support",     _NODBG_,  _mtlk_df_ui_scansupport_read  },
  { "scan_request",     _NODBG_,  _mtlk_df_ui_scanrequest_read  },
  { "surveys",          _NODBG_,  _mtlk_df_ui_survey_read       },
  { "version",          _NODBG_,  _mtlk_df_version_dump         },
  { "qos_map",          _NODBG_,  _mtlk_df_qos_map_dump         },
  { "PeerFlowStatus",   _NODBG_,  _mtlk_df_peer_flow_status     },
  { "corrupted_packet", _NODBG_,  mtlk_df_ui_corrupted_packet   },
  { "trace_buffer",     _NODBG_,  _mtlk_df_ui_trace_buffer_read },
  /* WriteOnly */
  { "ResetStats",       _DEBUG_,  NULL, _mtlk_df_ui_reset_stats_proc    },
  /* SEQ */
  { "General",          _DEBUG_,  mtlk_df_ui_debug_general      },
  { "hs20",             _DEBUG_,  mtlk_df_ui_debug_hs20_info    },

  { "sta_list",         _DEBUG_,  mtlk_df_ui_sta_list           },
...
#endif
  { "widan_blacklist",  _NODBG_,  _mtlk_df_ui_widan_blacklist   },
  { "multi_ap_blacklist",_NODBG_, _mtlk_df_ui_multi_ap_blacklist},
  { "wds_wpa_sta_list", _NODBG_,  _mtlk_df_ui_wds_wpa_sta_list  },
  { "hw_reg_dump",      _NODBG_,  _mtlk_df_ui_hw_reg_dump       },
...
_mtlk_df_unreg_file (mtlk_df_user_t* df_user, const struct _proc_file_info *info)
{
  if (info->show_func != NULL || info->write_func != NULL)
    mtlk_df_proc_node_remove_entry(info->name,
                                   info->is_dbg ?
                  df_user->slow_ctx->proc_df_debug_node :
                  df_user->slow_ctx->proc_df_node);
}
...
int __MTLK_IFUNC
mtlk_df_ui_linux_ioctl_stop_lower_mac (struct net_device *ndev,
                                       struct iw_request_info *info,
                                       union iwreq_data *wrqu,
                                       char *extra)
{
  ...
  res = _mtlk_df_user_invoke_core(df_user->df, WAVE_RADIO_REQ_STOP_LM, &clpb, NULL, 0);
  res = _mtlk_df_user_process_core_retval_void(res, clpb, WAVE_RADIO_REQ_STOP_LM, TRUE);
...
int __MTLK_IFUNC
mtlk_df_ui_linux_ioctl_setessid (struct net_device *ndev,
                           struct iw_request_info *info,
                           union iwreq_data *wrqu,
                           char *extra)
{
  ...
  * the ESSID can be passed without null terminated character*/
  res = _mtlk_df_user_iwpriv_set_param(df_user, PRM_ID_ESSID, extra, MTLK_IW_PRIW_PARAM(TEXT_ITEM, wrqu->essid.length));
...
int __MTLK_IFUNC
mtlk_df_ui_linux_ioctl_setfreq (struct net_device *ndev,
                                struct iw_request_info *info,
                                union iwreq_data *wrqu,
                                char *extra)
{
  int res = MTLK_ERR_NOT_SUPPORTED;
  ILOG1_S("%s: Channel selection unsupported via IOCTL", ndev->name);
...
int __MTLK_IFUNC
mtlk_df_ui_linux_ioctl_setretry (struct net_device *ndev,
                                 struct iw_request_info *info,
                                 union iwreq_data *wrqu,
                                 char *extra)
{
  ...
  case IW_RETRY_LIMIT | IW_RETRY_MAX:
    ...
    break;
  default:
    WLOG_SD("%s: Unknown parameter type: 0x%04x", ndev->name, wrqu->retry.flags);
    goto finish;
  }

  if (value >= 0) {
    ILOG2_DD("Set Retry limits (lifetime) to value %i %i ", value, wrqu->retry.value);
    res = _mtlk_df_user_iwpriv_set_param(df_user, obj_id, (char*)&value, MTLK_IW_PRIW_PARAM(INT_ITEM,1));
  }
                                 ...
                                 char *extra)
{
  ...
  /* Fill the supported channel info */
  {
    ...
    mtlk_df_t *m_df = mtlk_vap_manager_get_master_df(vap_mgr);
    mtlk_vap_handle_t m_vap_handle = mtlk_df_get_vap_handle(m_df);
    int cur_band = WAVE_RADIO_PDB_GET_INT(wave_vap_radio_get(m_vap_handle), PARAM_DB_RADIO_FREQ_BAND_CUR);
    /* before we have set the channel this may be incorrect */
    struct ieee80211_supported_band *band = wiphy->bands[mtlkband2nlband(cur_band)];
    int i;
...
/* This fucntion is used only to set WDS WEP keys */
int __MTLK_IFUNC
mtlk_df_ui_linux_ioctl_setenc (struct net_device *ndev,
                               struct iw_request_info *info,
                               union  iwreq_data *wrqu,
                               char   *extra)
{
  ...
  /* Mark values as not specified */
  enc_cfg.wep_enabled = MAX_UINT8;
  enc_cfg.key_id = MAX_UINT8;
  ...
  if (wrqu->data.flags & IW_ENCODE_DISABLED) {
    ...
    /* auto mode (on STA) */
    ILOG1_V("WDS doesn't support auto mode");

  } else if (wrqu->data.flags & IW_ENCODE_RESTRICTED) {
    /* Restricted mode */
    ILOG1_V("WDS doesn't support restricted mode");
  }

  /* Validate and adjust key index
     Up to 4 WEP keys supported. WE enumerate WEP keys from 1
     to N, and 0 means key not specified, current TX key will be used instead. */
  key_id = (uint8)(wrqu->data.flags & IW_ENCODE_INDEX);
  if (key_id != 0){
    if (MIB_WEP_N_DEFAULT_KEYS < key_id ) {
      WLOG_S("%s: Unsupported WEP key index", ndev->name);
      res = MTLK_ERR_PARAMS;
      goto finish;
    }
  ...
  if (0 != wrqu->data.length){
    if (mtlk_df_ui_validate_wep_key(extra, wrqu->data.length) != MTLK_ERR_OK) {
      WLOG_S("%s: Bad WEP key", ndev->name);
      res = MTLK_ERR_PARAMS;
      goto finish; /* res = -EINVAL */
    }
    /* Only one key might be passed from OS. So, slot 0 is used */
    enc_cfg.wep_keys.sKey[0].u8KeyLength = wrqu->data.length;
    wave_memcpy(enc_cfg.wep_keys.sKey[0].au8KeyData, sizeof(enc_cfg.wep_keys.sKey[0].au8KeyData),
        extra, wrqu->data.length);
  }

  res = _mtlk_df_user_invoke_core(df_user->df, WAVE_CORE_REQ_SET_WDS_WEP_ENC_CFG,
...
int __MTLK_IFUNC
mtlk_df_ui_linux_ioctl_getenc (struct net_device *ndev,
                               struct iw_request_info *info,
                               union  iwreq_data *wrqu,
                               char   *extra)
{
  ...
  res = _mtlk_df_user_invoke_core(df_user->df, WAVE_CORE_REQ_GET_WDS_WEP_ENC_CFG, &clpb, NULL, 0);
  res = _mtlk_df_user_process_core_retval_void(res, clpb, WAVE_CORE_REQ_GET_WDS_WEP_ENC_CFG, FALSE);
  if(MTLK_ERR_OK != res) {
    goto finish;
  }
  ...
  if (key_index != 0) {
    /* Report key only if key index is specified */
    key_index--;

  }else {
    /* Key index == 0 report current WEP configuration */
    wrqu->data.flags = 0;
  ...
  wave_memcpy(extra, wrqu->data.length, enc_cfg_res->wep_keys.sKey[key_index].au8KeyData, wrqu->data.length);
...
int __MTLK_IFUNC
mtlk_df_ui_linux_ioctl_setauth (struct net_device *ndev,
                                struct iw_request_info *info,
                                union  iwreq_data *wrqu,
                                char   *extra)
{
  ...
  ILOG2_SDD("%s: flags %i, value %i", ndev->name, wrqu->param.flags, wrqu->param.value);
  ...
  if (MTLK_ERR_NOT_SUPPORTED == res) {
    ...
    res = _mtlk_df_user_invoke_core(df_user->df, WAVE_CORE_REQ_SET_AUTH_CFG,
...
_mtlk_df_user_cleanup(mtlk_df_user_t *df_user)
{
  ...
  MTLK_CLEANUP_BEGIN(df_user, MTLK_OBJ_PTR(df_user))
    ...
    MTLK_CLEANUP_STEP(df_user, REGISTER_DEVICE, MTLK_OBJ_PTR(df_user),
                      _mtlk_df_user_unreg_netdev, (df_user));
    ...
    MTLK_CLEANUP_STEP(df_user, NETDEV_INIT, MTLK_OBJ_PTR(df_user),
                      MTLK_NOACTION, ());
...
static int
_mtlk_df_user_alloc_devname(mtlk_df_user_t *df_user, const char *name)
{
  int res;

  res = dev_alloc_name(df_user->ndev, name);
  ILOG0_SSS("dev_alloc_name %s, pattern:%s, name:%s",
            (res < 0) ? "failed" : "succeeded", name, df_user->ndev->name);
...
/**************************************************
 * netdev user-defined destructor
 * backport requires to define two functions:
 *
 * 1) void func_destructor(struct net_device *dev)
 *      - this function does main work
 *
 * 2) void __func_destructor(struct net_device *dev)
 *      - this function calls func_destructor(dev),
 *        then calls default destructor free_netdev(dev)
 */
void _mtlk_netdev_destructor (struct net_device *ndev)
{
  mtlk_df_user_p *df_user_pp;
  mtlk_df_user_t *df_user;
  ILOG1_S("Called destructor for %s", ndev->name);

  /* Get pointer to df_user */
  df_user_pp = (mtlk_df_user_p*)netdev_priv(ndev);
  /* Unlink net_device and df_user if pointer is not NULL */
  if (*df_user_pp) {
    df_user = *df_user_pp;
    *df_user_pp = NULL;
    ILOG2_S("Private data unlinked from %s", ndev->name);
    if ( df_user->destructor_flags & WAVE_DESTRUCT_DFUSER_DELETE_CALLED) {
      /* if mtlk_df_user_delete() was already called, simply release the memory
       * occupied by df_user */
      mtlk_osal_mem_free(df_user);
      ILOG2_S("Private data for %s deleted", ndev->name);
    } else {
      /* if mtlk_df_user_delete() wasn't yet called, break the link and set the special flag: */
      df_user->ndev = NULL;
      df_user->destructor_flags |= WAVE_DESTRUCT_NETDEV_DESTRUCTOR_CALLED;
      ILOG2_S("%s unlinked from private data", ndev->name);
    }
  }
}

void ___mtlk_netdev_destructor (struct net_device *ndev)
{
  _mtlk_netdev_destructor(ndev);
  free_netdev(ndev);
}

static int
_mtlk_df_user_netdev_init(mtlk_df_user_t *df_user, void *ctx, const char *name)
{
  int res;
  enum nl80211_iftype iftype;
  struct net_device *ndev = df_user->ndev;
  mtlk_vap_handle_t vap_handle = mtlk_df_get_vap_handle(df_user->df);
  bool is_ap = mtlk_vap_is_ap(vap_handle);

  /* For station mode Vifs netdev initialization is performed by mac80211 */
  if (!is_ap) {
    wave_strcopy(df_user->name, name, IFNAMSIZ);
    ILOG0_S("STA mode netdev name:%s", df_user->name);
    return MTLK_ERR_OK;
  }

  MTLK_ASSERT(NULL != ctx);

  res = _mtlk_df_user_alloc_devname(df_user, name);
  if (res != MTLK_ERR_OK) {
    return res;
  }

  wave_strcopy(df_user->name, ndev->name, IFNAMSIZ);

  /* link network device with wireless device */
  ndev->ieee80211_ptr = &df_user->wdev;

  /* initialize wireless device */
  df_user->wdev.wiphy = (struct wiphy *)ctx;

  /* configure interface type according to work mode */
  iftype = _get_iftype_by_role(df_user);
  df_user->wdev.iftype = iftype;

  /* link network device with parent device of wireless hardware */
  SET_NETDEV_DEV(ndev, wiphy_dev(df_user->wdev.wiphy));

  /* initialize Linux network device operational table */
  /* For station role interfaces we are using mac80211 ops*/
  _mtlk_df_user_fill_callbacks(df_user);

  /* setup destructor */
  if (is_ap) {
    netdev_set_priv_destructor(ndev,_mtlk_netdev_destructor);
  } else {
    netdev_set_def_destructor(ndev);
  }

  /* Configure core and radio iwpriv handlers */
  if(mtlk_vap_is_master_ap(vap_handle)) {
      ndev->wireless_handlers = (struct iw_handler_def *)&wave_linux_iw_radio_handler_def;
  } else {
      wave_linux_iw_core_handler_def.num_private_args = wave_df_get_core_privtab_size();
      ndev->wireless_handlers = (struct iw_handler_def *)&wave_linux_iw_core_handler_def;
  }
                                  ...
                                  BOOL cac_started)
{
  ...
  df_user = mtlk_df_get_user(mtlk_df_user_get_master_df(df_user));
...
void mtlk_df_user_handle_radar_event (mtlk_df_user_t *df_user, struct wireless_dev *wdev,
...
static void mtlk_df_user_propagate_dfs_state (struct work_struct *work)
{
  ...
  mtlk_dlist_foreach(&df_user->propagate_dfs_state_list, entry, head) {
    ...
    if (dfs_state_cfg->event == NL80211_RADAR_DETECTED)
      wv_cfg80211_radar_event(wdev, &dfs_state_cfg->chandef, dfs_state_cfg->cac_started, dfs_state_cfg->rbm);
...
static int
_mtlk_df_user_init(mtlk_df_user_t *df_user, void *ctx, const char *name)
{
  int i;
  ...
  MTLK_INIT_TRY(df_user, MTLK_OBJ_PTR(df_user))
    ...
    MTLK_INIT_STEP(df_user, NETDEV_INIT, MTLK_OBJ_PTR(df_user),
                   _mtlk_df_user_netdev_init, (df_user, ctx, name));
...
#if _REG_NETDEV_WORKAROUND_ /* register on init, unregister on stop */
    /* WAVE600: TODO: moved from start. Check this approach later */
    MTLK_INIT_STEP(df_user, REGISTER_DEVICE, MTLK_OBJ_PTR(df_user),
                   _mtlk_df_user_reg_netdev, (df_user));
#endif
#if MTLK_USE_PUMA6_UDMA
...
mtlk_df_user_t*
mtlk_df_user_create(mtlk_df_t *df, void *ctx, const char *name, struct net_device *ndev_p)
{
  struct net_device *ndev = ndev_p;
  mtlk_df_user_t* df_user;
  bool is_ap = mtlk_vap_is_ap(mtlk_df_get_vap_handle(df));
  ...
  /* Create net_device for cfg80211 AP role VAP.
   * Use mac80211 allocated ndev for station role virtual interface*/
  if (is_ap) {
    mtlk_df_user_p *df_user_pp;

    ndev = alloc_etherdev(sizeof(mtlk_df_user_p));
    if (NULL == ndev) {
    ...
    /* Set link with df_user to net_device private data */
    df_user_pp = (mtlk_df_user_p*)netdev_priv(ndev);
    *df_user_pp = df_user;
  }
  ...
  if (MTLK_ERR_OK != _mtlk_df_user_init(df_user, ctx, name)) {
    if (is_ap) {
      ___mtlk_netdev_destructor(ndev);
    }
    mtlk_osal_mem_free(df_user);
    return NULL;
  }
...
mtlk_df_user_delete(mtlk_df_user_t* df_user)
{
  ...
  if (is_ap){
    if (df_user->ndev) {
      df_user->destructor_flags |= WAVE_DESTRUCT_DFUSER_DELETE_CALLED;
      ILOG2_V("netdev not yet unlinked; give a chance to kernel to delete private data later..");
    } else {
      mtlk_osal_mem_free(df_user);
      ILOG2_V("netdev already unlinked; private data deleted");
    }
  } else {
    /* For the STA interface, we just need to release memory allocated for df_user  */
    mtlk_osal_mem_free(df_user);
  }
}
    ...
    mtlk_df_user_t *df_user, uint32 index, char *pdata, uint32 datalen)
{
  ...
  if (index >= (df_user->slow_ctx->dbg_rr_addr_num + 1)) { /* +1 for header */
    ...
    rslt = snprintf(mac_addr, sizeof(mac_addr), MAC_PRINTF_FMT,
...
_mtlk_df_ui_dp_xmit (struct net_device *ndev, mtlk_core_handle_tx_data_t *data, struct dp_subif *tx_subif)
{
  ...
  if (__LIKELY(res == DC_DP_SUCCESS)) {
    ...
    ELOG_D("dc_dp_xmit error code (%d)", res);
...
mtlk_df_ui_dp_prepare_and_xmit (mtlk_vap_handle_t vap_handle, mtlk_core_handle_tx_data_t *data, BOOL wds, uint16 sta_sid, int mc_index, uint32 nbuf_flags)
{
    ...
    if ((MTLK_DC_DP_MODE_SWPATH == mtlk_hw_mmb_get_dp_mode(hw)) && (NULL != data->rx_subif))
    ...
    return _mtlk_df_ui_dp_xmit(mtlk_df_user_get_ndev(mtlk_df_get_user(mtlk_vap_get_df(vap_handle))), data, &tx_subif);
}
...
#if DC_DP_API_VERSION_CODE >= DC_DP_API_VERSION(4,0,0)
    if (mtlk_core_dp_get_subif_cb(vap_handle, mac_addr, &subif) == MTLK_ERR_OK) {
    ...
    uint32_t flags)
{
  ...
  if (0 == dc_dp_get_mark_pkt(skb, 0)) {
    ...
    ILOG3_V("Update device specific info");
...
{
    switch (mode) {
      case MTLK_DC_DP_MODE_FASTPATH_GRX350:  return "GRX350/550 FastPath";
      case MTLK_DC_DP_MODE_FASTPATH_GRX750:  return "GRX750/PUMA7 FastPath";
...
int __MTLK_IFUNC
mtlk_df_dcdp_get_host_capability (struct dc_dp_host_cap *dp_cap)
{
  MTLK_ASSERT(dp_cap);
  ...
  if (dp_cap->fastpath.support) {
    ILOG2_D("supported HW DC mode: %u", dp_cap->fastpath.hw_dcmode);
...
mtlk_df_dcdp_datapath_dev_init (mtlk_hw_t *hw, mtlk_dcdp_dev_t *dp_dev)
{
  ...
  if (MTLK_ERR_OK == mtlk_df_dcdp_get_host_capability(&dp_dev->dp_cap)) {
...
/* Register and initialize datapath device */
{
  ...
  if (MTLK_DC_DP_MODE_UNREGISTERED == dp_dev->dp_mode)
  {
    ...
    if (DC_DP_FAILURE != dp_dev->dp_port_id[0]) {
      ...
      dp_dev->dp_port_id[0] = dc_dp_alloc_port(THIS_MODULE, dev_port, dp_dev->dp_ndev[0], 0, flags);
      TRACE_PARAM_INT(dp_dev->dp_port_id[0]);
      if (DC_DP_FAILURE == dp_dev->dp_port_id[0]) {
      ...
      if(is_cdb) {
        if (DC_DP_F_FASTPATH & flags) {
          ...
          dp_dev->dp_port_id[1] = dc_dp_alloc_port(THIS_MODULE, dev_port, dp_dev->dp_ndev[1], 0, flags);
        }
        TRACE_PARAM_INT(dp_dev->dp_port_id[1]);
    ...
    ILOG2_V("Input params for registration:");
    ...
    dp_dev->dp_resources.num_bufs_req = dp_init->rd_pool_size;
     ...
     */
    dp_dev->dp_resources.rings.soc2dev.size       = dp_init->tx_ring_size;
    dp_dev->dp_resources.rings.soc2dev.flags      = DC_DP_F_PKTDESC_ENQ;
    dp_dev->dp_resources.rings.soc2dev_ret.size   = dp_dev->dp_resources.rings.soc2dev.size;
    dp_dev->dp_resources.rings.soc2dev_ret.flags  = DC_DP_F_PKTDESC_RET; /* | DC_DP_F_RET_RING_SAME_AS_ENQ;*/
    dp_dev->dp_resources.rings.dev2soc.size       = dp_init->rx_ring_size;
    dp_dev->dp_resources.rings.dev2soc.flags      = DC_DP_F_PKTDESC_ENQ;
    dp_dev->dp_resources.rings.dev2soc_ret.size   = dp_dev->dp_resources.rings.dev2soc.size;
    ...
    */
    ...
    else if ((dp_dev->dp_devspec[0].dc_accel_used == DC_DP_ACCEL_FULL_OFFLOAD) && (dp_dev->dp_devspec[0].dc_tx_ring_used == DC_DP_RING_HW_MODE0))
      dp_dev->dp_mode = MTLK_DC_DP_MODE_FASTPATH_GRX350;
    else if ((dp_dev->dp_devspec[0].dc_accel_used == DC_DP_ACCEL_FULL_OFFLOAD) && (dp_dev->dp_devspec[0].dc_tx_ring_used == DC_DP_RING_HW_MODE1))
      dp_dev->dp_mode = MTLK_DC_DP_MODE_FASTPATH_GRX750;
