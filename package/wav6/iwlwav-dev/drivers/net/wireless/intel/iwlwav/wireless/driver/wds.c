...
static void _wds_peer_remove_delete (wds_peer_t *peer)
{
  ...
  ILOG2_DY("CID-%04x: WDS peer delete ctx %Y", mtlk_vap_get_oid(wds->vap_handle), peer->addr.au8Addr);
...
wds_beacon_process (mtlk_handle_t master_wds_handle, const void *data,  uint32 data_size)
{
  ...
  if (NULL == on_beacon) /*peer migh be already deleted */
    goto end;
  ...
  if ((peer->state == WDS_STATE_DISCONNECTED) && mtlk_osal_atomic_get(&peer->can_connect)) {
    ...
    action = WDS_BEACON_CONNECT;
      ...
      if (beacon_data->is_he) {
        is_he_cap_changed = memcmp(peer->beacon_data.he_cap_info, beacon_data->he_cap_info, beacon_data->he_cap_info_len);
        if (is_he_cap_changed) {
            mtlk_dump(2, peer->beacon_data.he_cap_info, peer->beacon_data.he_cap_info_len, "peer:");
            mtlk_dump(2, beacon_data->he_cap_info, beacon_data->he_cap_info_len, "beacon:");
        }
      }
