...
static int _mtlk_df_init(mtlk_df_t *df, mtlk_vap_handle_t vap_handle, void *ctx, const char *name, struct net_device *ndev)
{
  MTLK_INIT_TRY(df, MTLK_OBJ_PTR(df))
    df->vap_handle  = vap_handle;
    MTLK_INIT_STEP_EX(df, USER, MTLK_OBJ_PTR(df),
                      mtlk_df_user_create, (df, ctx, name, ndev), df->df_user,
...
mtlk_df_t* mtlk_df_create(mtlk_vap_handle_t vap_handle, void *ctx, const char *name, struct net_device *ndev)
{
  ...
  if(MTLK_ERR_OK != _mtlk_df_init(df, vap_handle, ctx, name, ndev))
  {
