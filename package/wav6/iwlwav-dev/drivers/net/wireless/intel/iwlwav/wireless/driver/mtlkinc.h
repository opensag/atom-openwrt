...
#define SIZEOF(a) ((size_t)sizeof(a)/sizeof((a)[0]))
...
#define FIXP_TO_INT(fixp, nbi, nbf) \
            (((int)((fixp) << (BITS_PER_INT - ((nbi) + (nbf))))) \
                           >> (BITS_PER_INT - ((nbi) + (nbf))))
...
#else
#define __MTLK_INT_HANDLER_SECTION
#endif
