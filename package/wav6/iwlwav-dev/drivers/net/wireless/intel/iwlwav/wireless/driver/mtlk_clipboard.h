...
#define MTLK_CLPB_TRY_EX(clpb_data, clpb_data_size, data_size) {  \
  int __res;                        \
  MTLK_ASSERT(NULL != (clpb_data)); \
  if (!mtlk_clpb_check_data(clpb_data, data_size, clpb_data_size, __FUNCTION__, __LINE__ )) {\
...
BOOL __MTLK_IFUNC
mtlk_clpb_check_data (void *clpb_data, size_t data_size, size_t size_of_data, const char* fn_name, int n_line);
