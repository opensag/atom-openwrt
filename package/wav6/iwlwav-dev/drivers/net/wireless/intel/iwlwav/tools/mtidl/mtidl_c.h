...
/* Types */
typedef uint8     mtidl_byte_t;
typedef int8      mtidl_sbyte_t;
typedef uint16    mtidl_short_t;
typedef int16     mtidl_sshort_t;
...
#define MTIDL_ENTRY_START(friendly_name, type, level, source, id, description)  /* nothing */
#define MTIDL_ENTRY_END(type_name)                                              /* nothing */

#define MTIDL_ITEM_STRUCT_START(friendly_name, type, level, source, id, description) \
  typedef struct { 

#define MTIDL_ITEM_STRUCT_END(type_name) \
...
#define MTIDL_SBYTEVAL(name, description) \
  mtidl_sbyte_t name;

#define MTIDL_BYTEVAL(name, description) \
  mtidl_byte_t name;

#define MTIDL_SSHORTVAL(name, description) \
  mtidl_sshort_t name;

#define MTIDL_SHORTVAL(name, description) \
  mtidl_short_t name;
...
#define MTIDL_BYTEVAL_ARRAY(name, size, description) \
  mtidl_byte_t name[size];

#define MTIDL_SBYTEVAL_ARRAY(name, size, description) \
  mtidl_sbyte_t name[size];

#define MTIDL_SSHORTVAL_ARRAY(name, size, description) \
  mtidl_sshort_t name[size];

#define MTIDL_SHORTVAL_ARRAY(name, size, description) \
  mtidl_short_t name[size];

#define MTIDL_BYTEVAL_ARRAY_2D(name, size1, size2, description) \
  mtidl_byte_t name[(size1)][(size2)];

#define MTIDL_SHORTVAL_ARRAY_2D(name, size1, size2, description) \
  mtidl_short_t name[(size1)][(size2)];

#define MTIDL_LONGVAL_ARRAY_2D(name, size1, size2, description) \
  mtidl_long_t name[(size1)][(size2)];

#define MTIDL_LONGVAL_ARRAY(name, size, description) \
  mtidl_long_t name[size];

#define MTIDL_SLONGVAL_ARRAY(name, size, description) \
