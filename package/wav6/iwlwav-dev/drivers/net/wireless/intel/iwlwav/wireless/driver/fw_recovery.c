...
typedef struct {
  ...
  atomic_t               is_restart_cac;    /* restart CAC-timer in case of Fast or Full Recovery */
...
static int __unified_rcvry_proc_entry_read_fill_header (char *buff, size_t size, wave_rcvry_card_cfg_t *pcard) {
  ...
  sprintf_written = snprintf(buff_tmp, (size -= sprintf_written), "INTL\n");
  if (sprintf_written <= 0 || sprintf_written >= size)
  ...
  for (i = 0; i < pcard->nof_files; ++i) {
    ...
    sprintf_written = snprintf(buff_tmp, (size -= sprintf_written), "%s\n%X\n", pcard->info[i].name, pcard->info[i].io_size);
    if (sprintf_written <= 0 || sprintf_written >= size)
      return MTLK_ERR_UNKNOWN;
    buff_tmp += sprintf_written;
  }
  sprintf_written = snprintf(buff_tmp, (size -= sprintf_written), "<<<<");
  if (sprintf_written <= 0 || sprintf_written >= size)
    return MTLK_ERR_UNKNOWN;
  buff_tmp += sprintf_written;
...
static int _unified_rcvry_proc_entry_read (char *page, off_t off, int count, void *data)
{
  ...
  if (header_size == 0 || total_size == 0 || off >= total_size)
    goto end;
  ...
  if (off < header_size) {
    ...
    if (off == 0 && count >= header_size) {
      int ret;
      ret = __unified_rcvry_proc_entry_read_fill_header(page, count, pcard);
      if (ret != MTLK_ERR_OK) {
      ...
      to_write = MIN(header_size - off, count);
      wave_memcpy(page, count, header_buff + off, to_write);
      mtlk_osal_mem_free(header_buff);
      res = to_write;
    }
    goto end;
  } else { /* the offset is after the header */
    uint32 dump_pos = off - header_size;
    uint32 to_write = 0;
    char *dump_file;
    ...
    mtlk_osal_mutex_acquire(fw_dump_mutex);
    if (*dump_file_p != NULL) {
      to_write = MIN(count, total_size - dump_pos);
      dump_file = (char*)(*dump_file_p);
      dump_file += dump_pos;
      wave_memcpy(page, count, dump_file, to_write);
...
#define __RCVRY_STATS_FAILED " recovery failed"
  ...
  written = snprintf(buff_tmp, (size -= written), __RCVRY_STATS_HEADER);
  if (written < 0 || written >= size)
    return MTLK_ERR_UNKNOWN;
  written = snprintf(buff_tmp += written, (size -= written), "        %d", stats->FastRcvryProcessed);
  if (written < 0 || written >= size)
    return MTLK_ERR_UNKNOWN;
  written = snprintf(buff_tmp += written, (size -= written), __RCVRY_STATS_NUM_OF "FAST" __RCVRY_STATS_SUCCESS "\n");
  if (written < 0 || written >= size)
    return MTLK_ERR_UNKNOWN;

  written = snprintf(buff_tmp += written, (size -= written), "        %d", stats->FullRcvryProcessed);
  if (written < 0 || written >= size)
    return MTLK_ERR_UNKNOWN;
  written = snprintf(buff_tmp += written, (size -= written), __RCVRY_STATS_NUM_OF "FULL" __RCVRY_STATS_SUCCESS "\n");
  if (written < 0 || written >= size)
    return MTLK_ERR_UNKNOWN;

  written = snprintf(buff_tmp += written, (size -= written), "        %d", stats->FastRcvryFailed);
  if (written < 0 || written >= size)
    return MTLK_ERR_UNKNOWN;
  written = snprintf(buff_tmp += written, (size -= written), __RCVRY_STATS_NUM_OF "FAST" __RCVRY_STATS_FAILED "\n");
  if (written < 0 || written >= size)
    return MTLK_ERR_UNKNOWN;

  written = snprintf(buff_tmp += written, (size -= written), "        %d", stats->FullRcvryFailed);
  if (written < 0 || written >= size)
    return MTLK_ERR_UNKNOWN;
  written = snprintf(buff_tmp += written, (size -= written), __RCVRY_STATS_NUM_OF "FULL" __RCVRY_STATS_FAILED);
  if (written < 0 || written >= size)
    return MTLK_ERR_UNKNOWN;
  *(buff_tmp += written) = '\n';
...
static int _rcvry_stats_proc_entry_read (char *page, off_t off, int count, void *data)
{
  ...
  total_size += strlen(__RCVRY_STATS_HEADER);
  total_size += 4 * strlen(__RCVRY_STATS_NUM_OF);
  total_size += 2 * strlen(__RCVRY_STATS_SUCCESS);
  total_size += 2 * strlen(__RCVRY_STATS_FAILED);
...
int wave_rcvry_card_add (void *mmb_base, void *hw_ctx)
{
  ...
  pcard->hw_ctx = hw_ctx;
  snprintf(pcard->card_dir_name, sizeof(pcard->card_dir_name), "%s%d", WAVE_RCVRY_CARD_DIR_NAME, pcard->card_idx);
  ...
  rcvry_ctx->cfg.rcvry_fast_cnt = 5;
  rcvry_ctx->cfg.rcvry_full_cnt = 3;
  ...
  for (i = 0; i < pcard->nof_files; i++, info++, pcard->nof_files_done++) {
    ...
    snprintf(io_size_hex_buff, sizeof(io_size_hex_buff), "%X", info->io_size);
    header_size += strlen(info->name);
    header_size += strlen(io_size_hex_buff);
    header_size += 2; /* 2*LF */
  }
                                 ...
                                 const BOOL value)
{
  ...
  if (rcvry_ctx && _rcvry_is_initialized(rcvry_ctx)) {
    wave_rcvry_radio_ctx_t *radio_ctx = _rcvry_radio_ctx_get(rcvry_ctx, vap_manager);
    if (radio_ctx)
      atomic_set(&radio_ctx->is_restart_cac, (uint32)value);
  }
}
                                 ...
                                 const mtlk_vap_manager_t *vap_manager)
{
  ...
  if (rcvry_ctx && _rcvry_is_initialized(rcvry_ctx)) {
    wave_rcvry_radio_ctx_t *radio_ctx = _rcvry_radio_ctx_get(rcvry_ctx, vap_manager);
    if (radio_ctx)
      return (BOOL)atomic_read(&radio_ctx->is_restart_cac);
  }
  return FALSE;
}
...
static inline int __rcvry_step_fw_isolation (wave_rcvry_radio_ctx_t *radio_ctx, wave_rcvry_type_e rcvry_type)
{
  ...
  wave_radio_on_rcvry_isolate(wave_vap_radio_get(radio_ctx->master_vap_handle));
...
#if 0 /* TODO: check if this code is required */
  mtlk_core_t *master_core = mtlk_vap_get_core(rcvry->master_vap_handle);
...
static int _rcvry_task_master (mtlk_handle_t pcard_handle, const void *payload, uint32 size)
{
  ...
  if (RCVRY_TYPE_NONE == rcvry_type) {
    wave_rcvry_nl_send_msg_mac_error_evt(pcard,
                    __rcvry_mac_error_type_get(rcvry_ctx));
  }
  ...
  switch (rcvry_type)
  {
    ...
    case RCVRY_TYPE_UNRECOVARABLE_ERROR:
    {
      ...
      if (_wave_rcvry_krnl_panic_enabled(pcard)){
        char msg[128];
        snprintf(msg, 128, "Unrecovarable Error on card '%d'",pcard->card_idx);
...
static inline void __rcvry_task_initiate_print (wave_rcvry_task_ctx_t *rcvry_ctx, uint16 oid)
{
  ...
  ILOG0_DS("CID-%04x: %s Recovery initiated", oid,
           __rcvry_type_to_string(rcvry_type_current));
  ILOG0_DDDDD("CID-%04x: Fast: %d (%d), Full: %d (%d)", oid,
...
{
  return _rcvry_nl_send_msg(LTQ_NL80211_VENDOR_EVENT_ASSERT_DUMP_READY, pcard, NULL, 0, "Dump Ready event");
}
...
static void _rcvry_process_fw_log_buffers_on_exception (mtlk_hw_t *hw)
{
  uint8 is_supported_ex;

  wave_hw_mmb_get_prop(hw, MTLK_HW_FW_LOGGER_IS_EX, &is_supported_ex, sizeof(is_supported_ex));
  if (is_supported_ex)
    wave_hw_mmb_set_prop(hw, MTLK_HW_FW_LOG_BUFFER, &is_supported_ex, sizeof(is_supported_ex));
}
#endif
...
static void _rcvry_process_mac_fatal (wave_rcvry_card_cfg_t *pcard, const APP_FATAL *data)
{
  const char *const names[] = { /* APP_FATAL fields */
        ...
        "uEpcReg", "FileId", "GroupId", "OriginId", "FWinterface" };
...
#if (IWLWAV_RTLOG_FLAGS & RTLF_REMOTE_ENABLED)
  _rcvry_process_fw_log_buffers_on_exception(pcard->hw_ctx);
#endif

  ELOG_D("CID-%02x: APP_FATAL", pcard->card_idx);
  for (i = 0; i < (sizeof(APP_FATAL) / sizeof(val)); i++) {
