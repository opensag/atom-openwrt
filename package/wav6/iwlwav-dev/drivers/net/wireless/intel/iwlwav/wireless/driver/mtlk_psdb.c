...
wave_ant_params_t * __MTLK_IFUNC
...
__wave_psdb_is_radio_band_supported (hw_psdb_t *psdb, unsigned radio_idx, unsigned band)
{
  psdb_radio_cfg_t   *cfg = NULL;
  int                 res = FALSE;
...
BOOL __MTLK_IFUNC
wave_psdb_is_radio_band_supported (hw_psdb_t *psdb, unsigned radio_idx, unsigned band)
{
...
_psdb_dump_array (uint32 *wbuf, int nrows, int ncols)
{
    ...
    for (i = 0; i < nrows; i++) {
        wp = wbuf;
        res = snprintf(line, sizeof(line), "%4d:", i);
        if (!_psdb_check_snprintf(res, sizeof(line)))
            break;
        pos = res;
        for (j = 0; j < ncols; j++) {
            res = snprintf(line + pos, sizeof(line) - pos, " %08X", MAC_TO_HOST32(wbuf[j]));
...
static int
psdb_parse_regd_table (hw_psdb_t *psdb, psdb_any_table_t *regd_table)
{
...
#define _PWL_FACTOR_APLLY(value, factor)  if(0 != (value)) { (value) += (factor); }
    ...
    for (i = 0; i < ARRAY_SIZE(pw_limits->pw_limits); i++) {
      _PWL_FACTOR_APLLY(pw_limits->pw_limits[i], tx_ant_factor);
    }

#undef _PWL_FACTOR_APLLY

    /* If the MU/BF values are not configured, we need to use the OFDM values instead */
...
mtlk_psdb_get_rate_power_list (mtlk_core_t *core, mtlk_clpb_t *clpb)
{
    ...
    /* create returned data */
    for (i = 0; i < ARRAY_SIZE(psdb->rate_pw_table->rates_all); i++) {
...
static int
_mtlk_psdb_parse_cdb_cfg_table (hw_psdb_t *psdb, psdb_any_table_t *table)
{
  psdb_cdb_cfg_table_t  *cdb_table;
  wave_cdb_cfg_e         cfg;
  int res = MTLK_ERR_OK;
...
_mtlk_psdb_parse_cca_th_table (hw_psdb_t *psdb, psdb_any_table_t *table)
{
    ...
    res = _mtlk_psdb_check_table_sizes("CcaTh configuration",
              table, MTLK_CCA_TH_PARAMS_LEN, MTLK_HW_BAND_BOTH);
    if (MTLK_ERR_OK != res) {
      return res;
    }
    ...
    for (i = 0; i < MTLK_CCA_TH_PARAMS_LEN; i++) {
      cdb_table->cca_th_params[MTLK_HW_BAND_2_4_GHZ][i] = (int8)table->data.cca_th_entries[i].value_2G;
      cdb_table->cca_th_params[MTLK_HW_BAND_5_2_GHZ][i] = (int8)table->data.cca_th_entries[i].value_5G;
      ILOG2_DDDDD("CCA_TH[%d]: 0x%08X -> %d,  0x%08X -> %d", i,
          table->data.cca_th_entries[i].value_2G, cdb_table->cca_th_params[MTLK_HW_BAND_2_4_GHZ][i],
          table->data.cca_th_entries[i].value_5G, cdb_table->cca_th_params[MTLK_HW_BAND_5_2_GHZ][i]);
    }
...
static uint8
_wave_psdb_ant_mask_to_ant_sel_mask (uint8 ant_mask)
{
  uint8 sel_mask = 0;
  int   i;
...
static int
_mtlk_psdb_update_cdb_cfg_table (hw_psdb_t *psdb)
{
  psdb_cdb_cfg_table_t  *cdb_table;
  ...
  if (wave_cdb_cfg_is_not_set(radio_cfg->cdb_config)) {
    ...
    WLOG_DD("PSDB CDB config table missing. Use card config: band support 2.4G:%d  5G:%d",
            (int)cdb_table->band_support_2G, (int)cdb_table->band_support_5G);

    /* Card band support */
    if ((!cdb_table->band_support_2G) && (!cdb_table->band_support_5G)) {
  ...
  for (i = 0; i < cdb_table->nof_entries; i++) {
    ...
    radio_ant_cfg->tx_ant_sel_mask = _wave_psdb_ant_mask_to_ant_sel_mask(radio_ant_cfg->tx_ant_mask);
        ...
        if (fld_id >= PSDB_FIELD_TABLE_ID_MIN) {
            ...
            if (!tmptab) {
                ...
                if (PSDB_FIELD_BEAMFORM_CAPS == fld_id) { /* Beamforming capabilities */
                  ...
                  if (MTLK_ERR_OK != _mtlk_psdb_parse_cdb_cfg_table(psdb, tmptab)) {
                              ...
                              uint32 chip_id, uint32 hw_type, uint32 hw_rev)
{
    ...
    ILOG1_SD("Loaded '%s' of %d bytes", fw_info.fname, fb.size);
    ...
    res = _mtlk_psdb_update_cdb_cfg_table(psdb);
    if (MTLK_ERR_OK != res) { goto cleanup; }

     /* We sure that RegDomain table is found, so parse it */
    res = psdb_parse_regd_table(psdb, psdb->regd_table);
