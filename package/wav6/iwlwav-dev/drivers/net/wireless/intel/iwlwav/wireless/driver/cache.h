...
void __MTLK_IFUNC mtlk_cache_register_bss_nolock(scan_cache_t *cache, bss_data_t *bss);
...
static __INLINE void mtlk_cache_register_bss(scan_cache_t *cache, bss_data_t *bss)
{
  mtlk_osal_lock_acquire(&cache->lock);
  mtlk_cache_register_bss_nolock(cache, bss);
