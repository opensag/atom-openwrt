...
static rtlog_info_app_entry_t rtlog_info_app_array[] =
{
  ...
  {IWLWAV_RTLOG_APP_NAME_MTDUMP, IWLWAV_RTLOG_APPGROUP_OID_MTTOOLS, {IWLWAV_RTLOG_APP_PID_INVALID}},
...
static void
_mtlk_log_send_conf_on_cleanup (void)
{
  int res;
  rtlog_drv_app_msg_t msg;
  rtlog_app_drv_msghdr_t *hdr = &msg.hdr;
  ...
  res = __mtlk_log_send_notification_msg(&msg);
  if (MTLK_ERR_OK != res) {
    ELOG_DD("Unable to notify all applications about Driver's cleanup: "
...
mtlk_log_set_conf(char *conf)
{
  ...
  if (!(p = str = mtlk_osal_mem_alloc(strlen(conf) + 1, MTLK_MEM_TAG_LOGGER)))
    return;
  while (*conf) {
...
try_again:
  ...
  if (buf_space_left(p_log_data->unsafe_active_buf) >= buf_size) {
#else
...
mtlk_log_new_pkt_reserve_multi(uint32 pkt_size, mtlk_log_multi_buffer_t **ppdata)
{
  ...
  while (pkt_size > 0) {
    ...
    if (current_buffer == *ppdata                          /* if this is first packet part */
      ...
      buf       = p_log_data->unsafe_active_buf;
      part_size = MIN(pkt_size, buf_size);
    }
...
#define IWLWAV_RTLOG_DEBUG_PPA_XMIT 1    /* Debug printout packets from PPA */
...
static int _rtlog_ppa_port_id_get (void)
{
  ...
  if (g_rtlog_ndev_cfg.ppa.radio == NULL) {
    ...
    strncpy(g_rtlog_ndev_cfg.ppa.radio->name, rtlog_ppa_netdev_name,
      sizeof(g_rtlog_ndev_cfg.ppa.radio->name) - 1);
...
{
    _mtlk_log_send_conf_on_cleanup();
...
#define IWLWAV_RTLOG_WLAN_IF_INVALID   (~0)
