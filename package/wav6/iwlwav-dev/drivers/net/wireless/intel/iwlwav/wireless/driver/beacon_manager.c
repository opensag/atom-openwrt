...
#define IE_HE_MAC_TWT_RESPONDER_OFFSET     (WLAN_EID_EXTENSION_ID_OFFSET + 2) /* Extension length=1 */
...
static int _wave_beacon_man_template_push(mtlk_core_t *core, beacon_template_t *tmpl)
{
  ...
  if (err != MTLK_ERR_OK) {
    ...
    WLOG_DD("CID-%04x: UM_MAN_BEACON_SET_REQ failed, Status=%u", oid, psUmiBeacon->u8Status);
  }
...
/* beacon template update initiated by driver always called from master VAP serializer context */
int wave_beacon_man_beacon_update(mtlk_handle_t hcore, const void *data, unsigned data_size)
{
  mtlk_core_t           *core = HANDLE_T_PTR(mtlk_core_t, hcore); /* core of the VAP the beacon is changed for  */
  beacon_manager_priv_t *bmgr_priv;
  beacon_manager_t      *bmgr;
  beacon_template_t     *tmpl;
  mtlk_beacon_data_t     beacon_data;
  ...
  vap_idx   = mtlk_vap_get_id(core->vap_handle);
  ...
  /* init beacon data for push control */
  beacon_data.head = tmpl->ptr;
  beacon_data.tail = tmpl->ptr + tmpl->head_len;
  beacon_data.head_len = tmpl->head_len;
  beacon_data.tail_len = tmpl->tail_len;
