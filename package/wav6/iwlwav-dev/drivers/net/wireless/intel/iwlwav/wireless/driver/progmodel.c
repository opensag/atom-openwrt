...
_wave_progmodel_init (mtlk_txmm_t *txmm, wave_progmodel_t *fw, wave_radio_t *radio, int freq)
{
  ...
  MTLK_ASSERT(MTLK_CHIP_REV_CNT > fw->prgmdl_params.chip_revision);
  ...
  IF_CARD_G5      ( fw->prgmdl_params.count = GEN5_PROGMODEL_COUNT; );
                                 ...
                                 char *suffix1, char *suffix2)
{
  ...
  snprintf(fw->ff[num].fname, sizeof(fw->ff[0].fname), "ProgModel_%s_%s%s.bin",
...
_wave_progmodel_get_fname (wave_progmodel_t *fw)
{
  ...
  IF_CARD_G5 (
    chip_rev_idx = param->chip_revision;
    if (MTLK_CHIP_REV_NONE == chip_rev_idx) { /* backward compatibility */
      /* a special suffix for wrx514 */
      if (_mtlk_card_is_wrx514(param->hw_type, param->hw_revision)) {
        chip_rev_idx = MTLK_CHIP_REV_WRX514;
      } else {
        chip_rev_idx = (param->freq == MTLK_HW_BAND_2_4_GHZ) ? MTLK_CHIP_REV_WRX300 : MTLK_CHIP_REV_WRX500;
      }
    }

    pm_suffix = pm_suffixes[chip_rev_idx];

    /* file 0: common progmodel */
    _wave_progmodel_compose_ff_data(fw, 0, MTLK_FILE_TYPE_HW, pm_suffix, fpga_suffix);
  );
