...
/* minimal statistic */
  ...
  MTLK_CORE_CNT_MAN_FRAMES_CFG80211_FAILED,
...
#define MTLK_CORE_CNT_MAN_FRAMES_CFG80211_FAILED_ALLOWED                            MTLK_WWSS_WLAN_STAT_ID_MAN_FRAMES_CFG80211_FAILED_ALLOWED
...
typedef struct _mtlk_core_general_stats_t {
  ...
  uint32 rx_man_frames_cfg80211_fail;
...
#define CORE_MAX_KEY_LEN    32    /* from linux define WLAN_MAX_KEY_LEN */
#define CORE_KEY_SEQ_LEN     8    /* Key sequence length */
#define CORE_NUM_WEP_KEYS    4    /* MAX number of wep KEYS */
#define CORE_NUM_GROUP_KEYS  2    /* MAX number of wep KEYS */

typedef struct _core_key_t
{
  uint8   key[CORE_MAX_KEY_LEN];  /* Key data */
  uint8   key_len;                /* Key length */
  uint8   seq[CORE_KEY_SEQ_LEN];  /* key sequence */
  uint8   seq_len;                /* sequnece number length */
  uint32  cipher;                 /* CipherSuite for this key */
} core_key_t;

#define MAX_SCAN_WAIT_RETRIES 600
#define MAX_VAP_WAIT_RETRIES 600
#define MAX_AMSDU_NUM     7 /* AMSDU_NUM range */
#define MIN_AMSDU_NUM     2
...
struct nic_slow_ctx {
  ...
  scan_cache_t       cache; // FIXME likely unnecessary but too much code wants it

  mtlk_osal_timer_t sched_scan_timer;
  mtlk_osal_timer_t cac_timer;
  struct mtlk_scan_request *sched_scan_req;

  /* actual BSS data */
  bss_data_t         bss_data;

  // 802.11i (security) stuff
  core_key_t    keys[CORE_NUM_WEP_KEYS];
  ...
  core_key_t    wds_keys[CORE_NUM_WEP_KEYS];
...
struct nic {
  ...
  int net_state; // FIXME perhaps make it atomic if it even matters... see the next comment

  mtlk_osal_spinlock_t net_state_lock; // FIXME get rid of this, it's not used consistently for all accessess anyway
...
int __MTLK_IFUNC mtlk_core_get_dev_spec_info(mtlk_nbuf_t *nbuf, mtlk_core_dev_spec_info_t *dev_spec_info);

uint32 __MTLK_IFUNC mtlk_core_get_sid_by_addr(mtlk_core_t *nic, uint8 *addr);
...
int
__mtlk_core_add_blacklist_addr_entry (mtlk_core_t *nic, struct intel_vendor_blacklist_cfg *addr,
                                   ieee_addr_list_t *list, char *name);

int __MTLK_IFUNC mtlk_core_get_channel(mtlk_core_t *core);

#ifdef MTLK_LEGACY_STATISTICS
uint32 __MTLK_IFUNC mtlk_core_get_vap_stat(mtlk_core_t *nic, mtlk_mhi_stat_vap_e id);
uint64 __MTLK_IFUNC mtlk_core_get_vap_stat64(mtlk_core_t *nic, mtlk_mhi_stat64_vap_e id);
#else
