...
enum {
  ...
  PRM_ID_USE_SHORT_CYCLIC_PREFIX,
...
/* For AP, df_user is in net_device priv prt
 * For Station interface df_user is in wv_vif_priv */
void
...
mtlk_df_user_from_wdev (struct wireless_dev *wdev)
{
  if (wdev->iftype == NL80211_IFTYPE_AP)
    return *((mtlk_df_user_p*) netdev_priv(wdev->netdev));
  else
    return mtlk_df_user_ieee80211_ndev_to_dfuser(wdev->netdev);
}
...
mtlk_df_user_from_ndev (struct net_device *ndev)
{
  ...
  return mtlk_df_user_from_wdev(wdev);
}

static __INLINE mtlk_df_user_t *
mtlk_df_user_from_skb (struct sk_buff *skb)
{
  MTLK_ASSERT(NULL != skb);

  return mtlk_df_user_from_ndev(skb->dev);
}
...
mtlk_df_user_add_propagate_event (mtlk_df_user_t *df_user,
                                  ...
                                  BOOL cac_started);
void __MTLK_IFUNC
mtlk_df_user_handle_radar_event (mtlk_df_user_t *df_user, struct wireless_dev *wdev,
