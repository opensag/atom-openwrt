...
void wpa_printf(int level, const char *fmt, ...)
{
	...
	if (wpa_rtlogger_get_flags(level) & WPA_RTLOGGER_LOG_TARGET_REMOTE) {
		va_start(ap, fmt);
		wpa_rtlogger_remote_vlog(fmt, ap);
		va_end(ap);
	}
#endif /* !CONFIG_NO_WPA_RTLOGGER && !CONFIG_WPA_RTLOGGER_RUN_DEMO */
}
#endif /* CONFIG_SILENT */
