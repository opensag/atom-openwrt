...
/**
 * os_strlcat - Appends src to string dest of size siz and NUL-termination
 * @dest: Destination
 * @src: Source
 * @siz: Size of the target buffer
 * Returns: Total length of the target string  after append (length of src + min(siz, initial length of the target string))
 * (not including NUL-termination)
 *
 * This function matches in behavior with the strlcat function in OpenBSD.
 */
size_t os_strlcat(char *dest, const char *src, size_t siz);
