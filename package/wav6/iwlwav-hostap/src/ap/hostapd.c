...
static void hostapd_post_up_update_bss_defaults(struct hostapd_data *hapd, struct hostapd_bss_config *conf)
{
  struct intel_vendor_radio_info radio_info;
  hostapd_drv_get_radio_info(hapd, &radio_info);

  if((radio_info.Channel > 13) &&
      (radio_info.num_tx_antennas < 4)) {
    hapd->iface->conf->he_oper.bss_color_info = 5;
    if(MASTER_VAP_BSS_IDX == hapd->conf->bss_idx)
      hapd->iface->conf->he_oper.max_co_located_bssid_ind = 4;
  } else {
    hapd->iface->conf->he_oper.bss_color_info = 4;
    if(MASTER_VAP_BSS_IDX == hapd->conf->bss_idx)
      hapd->iface->conf->he_oper.max_co_located_bssid_ind = 5;
  }

  /* TEMPORARY forced */
  hapd->iface->conf->he_oper.bss_color_info = 4;

  wpa_printf(MSG_DEBUG, "hostapd_post_up_update_bss_defaults - Channel:%d, num_tx_antennas:%d, he_bss_color:%d, bss_idx:%d",
             radio_info.Channel,
             radio_info.num_tx_antennas,
             hapd->iface->conf->he_oper.bss_color_info,
             hapd->conf->bss_idx);
}
...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
  ...
  if (ltq_wds_wpa_init(hapd)) {
	...
	hostapd_post_up_update_bss_defaults(hapd, conf);
	...
	if (iface->conf->channel) {
		...
		if (!(iface->drv_flags & WPA_DRIVER_FLAGS_DFS_OFFLOAD)) {
			...
			if (iface->conf->dfs_debug_chan &&
				...
				iface->conf->dfs_dbg_cac_process = FALSE;

				if (iface->bss[0]->started)
					return ieee802_11_set_beacons(iface);
			}
		} else {
