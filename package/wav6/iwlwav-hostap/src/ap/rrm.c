		...
		u8 token, const u8 *pos, size_t len, const u8 *sta_addr)
{
	...
	int report_id = -1, fragment_id = -1, more_fragment_bit = -1;
	...
	if ((len <= 29) || (pos[29] != WLAN_BEACON_REPORT_SUBELEM_FRAME_BODY)) { /* No sub-elements */
		wpa_msg(hapd->msg_ctx, MSG_INFO, RRM_EVENT_BEACON_REP_RXED MACSTR
	...
	attr = get_ie(&pos[29], len, WLAN_BEACON_REPORT_SUBELEM_FRAME_BODY_FRAGMENT_ID);
	if (attr && attr[1] == 2 && attr + attr[1] < end) {
		report_id = attr[2];
		fragment_id = attr[3] & 0x7f; /* Bits 0 - 6 */
		more_fragment_bit = attr[3] >> 7; /* Bit 7 */
		os_snprintf(frame_body_fragment_id, 64,
				"report_id=%d fragment_id=%d more_fragment_bit=%d",
				report_id, fragment_id, more_fragment_bit);
	}

	/* initial fragment include additional 14 bytes */
	if (fragment_id == 0 || fragment_id == -1) {
		frame_body.length -= 14;
	...
	attr = get_ie(&pos[first_sub_elem_idx], len, WLAN_BEACON_REPORT_SUBELEM_LAST_INDICATION);
	if (attr && attr[1] == 1 && attr + attr[1] < end)
		os_snprintf(last_beacon_rep, 32, "last_beacon_rep=%d", attr[2]);
