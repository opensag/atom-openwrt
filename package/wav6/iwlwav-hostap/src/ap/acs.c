...
void add_160_channels(struct hostapd_iface *iface, channel_160 *channel_pairs, u32 nof_pairs, struct hostapd_channel_data *chan)
{
  ...
  for (i = 0; i < nof_pairs; i++) {
    if ((channel_pairs[i].primary == chan->chan) &&
      ...
      mode->candidates[mode->num_candidates].primary = channel_pairs[i].primary;
      mode->candidates[mode->num_candidates].secondary = channel_pairs[i].secondary;
      mode->candidates[mode->num_candidates].width = 160;
...
char acs_to_exclude_candidate(struct hostapd_iface *iface, u32 i, struct os_reltime *now)
{
  ...
  if (mode->candidates[i].overlap_vht_he &&
    !os_reltime_expired(now, &mode->candidates[i].ts_overlap_vht_he, iface->conf->acs_to_degradation[T_LNTO])) {
...
int acs_do_switch_channel(struct hostapd_iface *iface, int block_tx)
{
  ...
  for (i = 0; i < mode->num_channels; i++) {
    ...
    if (channels_overlap(channel->freq, 20, mode->candidates[mode->selected_candidate].freq,
                                            mode->candidates[mode->selected_candidate].width) &&
...
void acs_print_cand_no(int level, struct hostapd_hw_modes *mode, int i)
{
  ...
  mode->candidates[i].overlap160,
  mode->candidates[i].primary_on_overlap_4080160,
...
int acs_recalc_ranks_and_set_chan (struct hostapd_iface *iface, int switch_reason)
{
  ...
  for (i = 0; (i < mode->num_candidates && i < MAX_CANDIDATES); i++) {
    ...
    if (mode->candidates[i].exclude_reason == ACS_NO_EXCLUSION) {
      ...
      switch (mode->candidates[i].width) {
        ...
        case 80 : dyn_bwccs = mode->candidates[i].dynBW80;  break;
        case 160: /* fallthrough - 80p80 go here */
        default : dyn_bwccs = mode->candidates[i].dynBW160; break;
      }
      ...
      if (iface->conf->acs_vht_dynamic_bw)
        vht_dynamic_bw80 = iface->conf->acs_penalty_factors[K_DYN_BW] * MIN(100, AOCS_NORM_DYNAMIC_BW_DISABLED * mode->candidates[i].overlap80);
      else
        vht_dynamic_bw80 = iface->conf->acs_penalty_factors[K_DYN_BW] *
        MIN(100, AOCS_NORM_DYNAMIC_BW_ENABLED * mode->candidates[i].overlap80 * mode->candidates[i].primary_on_overlap_4080160);
      ...
      if (iface->conf->acs_vht_dynamic_bw)
        vht_dynamic_bw160 = iface->conf->acs_penalty_factors[K_DYN_BW] * MIN(100, AOCS_NORM_DYNAMIC_BW_DISABLED * mode->candidates[i].overlap160);
      else
        vht_dynamic_bw160 = iface->conf->acs_penalty_factors[K_DYN_BW] *
        MIN(100, AOCS_NORM_DYNAMIC_BW_ENABLED * mode->candidates[i].overlap160 * mode->candidates[i].primary_on_overlap_4080160);
      ...
      if (os_reltime_expired(&now, &mode->candidates[i].ts_update, iface->conf->acs_to_degradation[T_STO])) {
        ...
        ci = iface->conf->acs_penalty_factors[K_CI] *
          (pow(10, n / 5) + ((double)mode->candidates[i].busy_time / (double)mode->candidates[i].total_time) *
  ...
  for (j = 0; j < mode->num_candidates; j++)
  {
    ...
    if (mode->candidates[j].primary == ch_data->primary)
    {
      ...
      if (ch_data->filled_mask & CHDATA_NOISE_FLOOR) {
        ...
        if ((u32)ch_data->noise_floor < mode->min_noise) mode->min_noise = (u32)ch_data->noise_floor;
      }
      ...
      if (ch_data->filled_mask & CHDATA_DYNBW) { /* these counters are reset by FW */
        mode->candidates[j].dynBW160 += ch_data->dynBW160;
