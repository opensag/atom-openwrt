	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
	char *pos, *end;
	int ret;
	const u8 *ie, *ie2, *osen_ie, *p2p, *mesh, *owe,*ve_ie;
		...
		char *cmd, char *buf, size_t buflen)
{
	...
	ret = os_snprintf(pos, end - pos, "BytesSent=%llu\n",
			radio_info.traffic_stats.BytesSent);
	if (os_snprintf_error(end - pos, ret))
		return -1;
	pos += ret;

	ret = os_snprintf(pos, end - pos, "BytesReceived=%llu\n",
			radio_info.traffic_stats.BytesReceived);
	if (os_snprintf_error(end - pos, ret))
		return -1;
	pos += ret;

	ret = os_snprintf(pos, end - pos, "PacketsSent=%llu\n",
			radio_info.traffic_stats.PacketsSent);
	if (os_snprintf_error(end - pos, ret))
		return -1;
	pos += ret;

	ret = os_snprintf(pos, end - pos, "PacketsReceived=%llu\n",
			   ...
			   char *reply, int reply_size, int *reply_len)
{
	...
	os_memset(scan_id, 0, sizeof(scan_id));
					 ...
					 char *buf, size_t *resp_len)
{
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		reply_len = ap_ctrl_iface_sta_first(wpa_s, buf + 10,reply, reply_size);
